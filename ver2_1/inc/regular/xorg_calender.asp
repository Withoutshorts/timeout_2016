...
'******************************************* dato funktioner ***********************************
		...
		lastWeekday = Weekday(lastDayOfMonth) 
		...
		kalenderlink = "ketype=e&selpkt=kal&status="&status&"&id="&id&"&emner="&emner&"&medarb="&medarb&"&sort="&request("sort")&""
		showother = "notused"
		else
			if andre = "show" then 
...
<table cellspacing="0" cellpadding="0" border="0" width="200" bgcolor="#FFFFFF">
	<tr bgcolor="ffffff" height="25">
		<td>
		<img src="<%=illpath%>ill/pile_tilbage.gif" alt="" vspace="1" hspace="2" border="0"><a class="vmenu" href="<%=seldocument%>.asp?menu=<%=menu%>&FM_andre=<%=showother%>&strdag=<%=strDag%>&strmrd=<%=prevMonth%>&straar=<%=prevYear%>&<%=kalenderlink%>"><%=prevMonthName%></a>
		</td>
		<td align="center"><b><%=thisMonthName%>&nbsp;<%=strAar%></b></td>
		<td align="right">
		<a class="vmenu" href="<%=seldocument%>.asp?menu=<%=menu%>&FM_andre=<%=showother%>&strdag=<%=strDag%>&strmrd=<%=nextMonth%>&straar=<%=nextYear%>&<%=kalenderlink%>"><%=nextMonthName%></a><img src="<%=illpath%>ill/pile_selected.gif" alt="" vspace="1" hspace="2" border="0">
		</td>
		</tr>
</table>
<table cellspacing="0" cellpadding="0" border="0" width="200" bgcolor="#FFFFFF">
...
if menu = "timereg" OR menu = "" then
	...
	'** Henter crmaktioner ***
			strSQL3 = "SELECT crmhistorik.id, aktionsid, medarbid, kkundenavn, crmhistorik.navn, crmemne.navn AS emnenavn FROM crmhistorik, aktionsrelationer, crmemne, kunder WHERE crmdato = '"& strAar &"/" & strMrd & "/" & daysinFirstWeek &"' AND crmemne.id = kontaktemne AND Kid = kundeid "& usemedarbKri &" ORDER BY crmhistorik.id"
...
for dayCount = startsecondWeek to numberofdaysinmonth
	...
	strSQL = "SELECT sum(timer) AS timer_indtastet FROM timer WHERE Tmnr = "& varMed_id &" AND Tdato='"& strAar &"/" & strMrd & "/" & dayCount  &"' AND tfaktim <> 5"
		...
		'** Henter crmaktioner ***
			strSQL3 = "SELECT crmhistorik.id, aktionsid, medarbid, kkundenavn, crmhistorik.navn, crmemne.navn AS emnenavn FROM crmhistorik, aktionsrelationer, crmemne, kunder WHERE crmdato = '"& strAar &"/" & strMrd & "/" & dayCount &"' AND crmemne.id = kontaktemne AND Kid = kundeid "& usemedarbKri &" ORDER BY crmhistorik.id"
			oRec3.open strSQL3, oConn, 3
			akt_today = "n"
			strAktnavn = "Aktioner idag:" &vbcrlf
				while not oRec3.EOF
	...
	if Weekday(formatdatetime(dayCount &"/" & strMrd & "/" & strAar, 0)) = 1 then 
...
if strA4 = "A4 Se timereg for andre medarb. on" then
	if level <= 2 AND andre <> "show" OR level = 6 AND andre <> "show" then%>
	<%if request("menu") <> "crm" then%>
	<table cellspacing="0" cellpadding="0" border="0" width="200">
	<tr bgcolor="#5582D2">
		<td width="3"><img src="../ill/venstre_hjorne.gif" alt="" border="0"></td>
		<td width="170" valign="middle" style="border-top: 1px solid #003399;"><font class="stor-hvid">&nbsp;Se timeregistreringer for:</font></td><td style="border-top: 1px solid #003399;"><a href="#" onclick="expand2('vistimerformedarb');"><img src="../ill/pilned.gif" width="20" height="22" alt="" border="0"></a></td>
		<td width="3" align="right"><img src="../ill/hojre_hjorne.gif" alt="" border="0"></td>
	</tr>
	</table>
	<DIV ID="vistimerformedarb" name="vistimerformedarb" style="position: relative; display: ;">
	...
	<%
		...
		%>
		<tr bgcolor="#FFFFFF">
			<td style="border-left: 1px solid #003399;" width="15" valign="bottom"><a href="javascript:NewWin_cal('../inc/regular/calender.asp?FM_andre=show&FM_medarb_id=<%=oRec("Mid")%>&FM_Mnavn=<%=oRec("Mnavn")%>')"><img src="../ill/pile_selected.gif" alt="" border="0"></a></td>
			<td style="border-right: 1px solid #003399;" width="195" valign="bottom"><a href="javascript:NewWin_cal('../inc/regular/calender.asp?FM_andre=show&FM_medarb_id=<%=oRec("Mid")%>&FM_Mnavn=<%=oRec("Mnavn")%>')" class="vmenu"><%=oRec("Mnavn")%></a></td>
		</tr>
		<%
		oRec.movenext
		wend
