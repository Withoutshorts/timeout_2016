...
function fyldoptimertilnorm(lto, mid, forcerun, stdato, sldato)
        ...
        'if cint(forcerun) = 1 then 'kørt manuelt fra HR siden - find fredag i ugen før + 14 dage
        ...
        '** Kør alle dage Kør kun specifik dag
        '** Husk påskemandag, 2 juledag osv.
        ...
        if cint(runday) = 0 OR cint(runday) = cint(ddthisWeekDay) OR cint(forcerun) = 1 then
            ...
            for d = stpoint to endpoint 'last 13 OR 30 days STARTER fra igår
            ...
            case else
                    ...
                    oRec9.close
                        ...
                        '** LM
                        '** ALTID FYLD OP TIL Komme/Gå 
                        'if minutterThisDay <= ntimper then 'Er komme gå tid større end norm fyldes op hertil
...
function maksFlexControl(lto, mid, timertilindlasning)
        ...
        '*** 20190110 - Tænk over at gemme talle hver FY, da indlæsningern over tid vil blive langsom hvis den altid skal gå tilbage til 1.1.2018 / Licensstart
        'maksFlexControl(
        ...
        'meType = 40 
        '** Funktionære pr. uge
        '** Timelønnede pr. måned
        'call medariprogrpFn(mid)
      

        dd = now

                if cint(meType) = 40 then 'timelønnede
                'dd = dateAdd("d", -1, dd)
        ...
        aktFaktimType = 30
            ...
            '*** Nulstiller indlæst overarb. pr. uge **'
      ...
      '*** If maxflxprdag <> 0 then
       ...
       oConn.execute(strSqlDel)
        ...
        '*** tjekker flex saldo : slDatoSQL T.O.M iGÅR *** 
        realTimerFYmax10 = 0 
        ...
        '** beregnes indenfor en uge / Måned  **'
        if cint(meType) = 40 then 'timelønnede
        flexLoft = 37
        else
        flexLoft = 74
...
function indlasTimertilUdbetaling(lto, mid, timertilindlasning, akttype)
        ...
        call meStamdata(mid)
                                ...
                                '** Fradrag på Overarbejde 1:5 minus
                                select case lto
                                case "dencker"
                                        ...
                                        if cint(akttype) = 32 then
		                                    ...
		                                    & "'"& year(now) &"/"& month(now) & "/"& day(now) &"', 0, "_
		                                    & "'"& tidspunkt &"', "_
		                                    & "'Timeout tillæg indlæsning - fradrag', 0, 0, '00:00:00', '00:00:00', 1, 100, 482)"
                            ...
                            case "cflow", "intranet - local"
                                       ...
                                       'if lp = 0 then
                                                        ...
                                                        case 0 'Dag ordinær type 1
                                                                ...
                                                                case 6,7 'lørdag og søndag skal være 100% overtid alle timer
                                                                lb = "00:00:01"
                                                                ub = "23:59:59"
                                                                case else
                                                                lb = "07:30:00"
...
function indlasTimerTfaktimAktid(lto, mid, timertilindlasning, tfaktim, aktid, insertUpdate, insUpdDate, extsysid, timerkom, koregnr, destination, usebopal)
       ...
       'insertUpdate = 1 alm, 2 altid på dd., 3 tjek for tidligere indtasninger (Dencker faste tillæg / fradrag for mad der kun må tastwes en gang pr. dag)
        ...
        call meStamdata(mid)
                        ...
                        if cdbl(aktid) <> 0 AND cdbl(jobid) <> 0 then
                                ...
                                if cint(insertUpdate) = 3 then
                                    ...
                                    '*** Findes indtastning allerede **** f.eks faste tillæg Dencker, der ikke skal tilføjes hver gang der tastes.
		                        ...
		                        & "'"& year(now) &"/"& month(now) & "/"& day(now) &"', 0, "_
		                        & "'"& tidspunkt &"', "_
		                        & "'Timer indlæst', 0, 0, '00:00:00', '00:00:00', 1, 100, 781, "& extsysid &", '"& timerkom &"', '"& koregnr &"', '"& destination &"', "& usebopal &")"
...
function sygdom120(lto)
        ...
        '** Indlæser automatisk sygdom + 120 dages regel ***'
        dd = now
        dd_1 = dateadd("d", -1, dd)
        ...
        tidspunkt = formatdatetime(now, 3)
                                ...
                                '***** Er funktioen allerede kørt idag
                                korantalSygdom = 1
          ...
          if cint(korantalSygdom) = 1 then
                                ...
                                While not oRec7.EOF 
                                        ...
                                        erLastSygdom = 0
                                        '*** Er den sidste reg. en sygdom - indlæs da en mere
		                                ...
		                                & "'"& year(now) &"/"& month(now) & "/"& day(now) &"', 0, "_
		                                & "'"& tidspunkt &"', "_
		                                & "'Timeout sygdom indlæsning', 0, 0, '00:00:00', '00:00:00', 1, 100, 481)"
                                 ...
                                 '***** Oprettter Mail object ***
                                    ...
                                    'response.flush
			                                ...
			                                Set myMail=CreateObject("CDO.Message")
                                            myMail.Subject="TimeOut - Medarbejder "& meNavn &" nærmer sig 120 sygedages regel"
                                            myMail.From = "timeout_no_reply@outzource.dk"
                                            ...
                                            myMail.Cc= "jda@outzource.dk; sk@outzource.dk"
			                                        ...
			                                        ' Selve teksten
					                                txtEmailHtml = "<br>Medarbejder "& meNavn &" har nået "& antalSygdom &" sygedage d. "& weekdayname(weekday(dd_1, 1)) &" d. "& formatdatetime(dd_1, 2) 
                                                    txtEmailHtml = txtEmailHtml & "<br><br>Det er opnået i perioden: "& dd_121 &" til "& dd_1 
