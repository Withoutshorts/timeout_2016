...
if len(session("user")) = 0 then
			...
			isInt = 0
					...
					isInt = 0
							...
							'*** Hvis alle required er udfyldt ***'
								    ...
								    '*** Momsberegning ****'
									if cint(medregnikkeioms) = 1 then 'intern
                                    momsBelob = 0
                                    else
							...
							if func = "dbred" then
                                            ...
                                            &" hidefasesum = "& hidefasesum &", hideantenh = "& hideantenh &", medregnikkeioms = "& medregnikkeioms & ", afsender = " & afsender & ", vis_jobbesk = "& visjobbesk &", kontonr_sel = "& afs_bankkonto
											...
											oConn.execute(strSQLupd)
													...
													if aftid <> 0 then
                                                        ...
                                                        &" labeldato = '"& labelDato &"' WHERE faknr = "& intFaknum & " AND shadowcopy = 1"
							...
							if func = "dbopr" then
                                                ...
                                                fak_laast = 0
                                                    ...
                                                    &" visikkejobnavn, hidefasesum, hideantenh, medregnikkeioms, fak_laast, afsender, vis_jobbesk, kontonr_sel) VALUES ("_
													&" '"& intFaknum &"',"_
													&" '"& fakDato &"',"_
													&" "& jobid &","_
													&" "& intTimer &","_
                                                    ...
                                                    &" "& hidefasesum &", "& hideantenh &", "& medregnikkeioms &", "& fak_laast &", "& afsender &", "& visjobbesk &", "& afs_bankkonto &")")
													...
													for i = 0 TO UBOUND(job_tilknyttet_aftale, 1)
													    ...
													    strSQLshadowCopy = "INSERT INTO fakturaer (faknr, fakdato, jobid, shadowcopy, istdato, brugfakdatolabel, labeldato) VALUES "_
													    &"('"& intFaknum &"', '"& fakDato &"', "& job_tilknyttet_aftale(i) &", 1, '"& istDato &"',"& brugfakdatolabel &",'"& labelDato &"')"
    ...
    'redim thisaktidFlereTp(150)
	Redim thisaktid(150)
	Redim thisaktnavn(150)
	Redim thisAktTimer(150), thisAktTimerSum(150)
	...
	'*********************************************************************
						...
						&" sideskiftlinier, labeldato, fak_abo, fak_ubv, visikkejobnavn, hidefasesum, hideantenh, medregnikkeioms, fak_laast, afsender, vis_jobbesk, kontonr_sel"_
	...
	<%
		...
		&" jobstartdato, jobslutdato, jobfaktype, rekvnr, jobstatus, usejoborakt_tp, ski, abo, ubv, s.navn AS aftalenavn, aftalenr, serviceaft, job_internbesk, j.kommentar, jo_bruttooms, altfakadr FROM job j "_
		...
		if cint(chkemail) = 1 OR (lto = "jttek") then
		visafsemail = 1
		else
		visafsemail = 0
		end if
		...
		if cint(chktlffax) = 1 OR (lto = "epi") OR (lto = "epi_osl") OR (lto = "epi_sta") OR lto = "epi_ab" OR (lto = "intranet - local") then
		visAfsTlf = 1
		visAfsFax = 0
		else
		visAfsTlf = 0
        ...
        if (lto = "epi") OR (lto = "epi_osl") OR (lto = "epi_sta") OR (lto = "epi_ab") OR lto = "intranet - local" then
        intModCvr = 1
        else
		intModCvr = 0
		end if
        ...
        select case lto
        case "epi", "epi_osl", "epi_sta", "epi_ab"
        hideantenh = 1
        case else
        hideantenh = 0
        end select
	...
	%>
	              ...
	              <div id="sideload" style="position:absolute; top:161px; left:205px; display:; visibility:visible; border:3px #cccccc solid; padding:2px; background-color:#FFFFFF; padding:10px; width:300px; z-index:20000;">
                        ...
                        %>
                        <br />
                        Der er MERE end 100 aktive aktiviteter på dette job. Derfor kan fakturaen ikke vises.<br /><br /> Deaktiver de aktiviteter der ikke er ibrug og forsøg igen, eller kontakt support@outzource.dk<br />

                        <% 'respoonse.flush
                        else
	                    %>
	                    
	                    Der er <b><%=antalaktCount%> aktive aktiviteter</b>  på dette job, der via deres aktivitetstype er sat til at blive vist på faktura.<br /><br />
                        <b>Forventet loadtid:</b><br />
	                    Mellem: <%=formatnumber((antalaktCount*0.2), 0) %> og <%=formatnumber((antalaktCount*1.4), 0) %> </b> sek.
                        <%end if %>
	                ...
	                <div id=knap_joblogdiv style="position:absolute; visibility:visible; display:; top:-30px; width:50px; left:413px; border:1px #8cAAe6 solid; padding:3px 3px 3px 3px; background-color:#FFFFFF;">
                    <a href="#" onclick="showdiv('joblogdiv')" class=vmenu>Joblog</a>
	                </div>
	                
	                <div id=knap_matlogdiv style="position:absolute; visibility:visible; display:; top:-30px; width:150px; left:473px; border:1px #8cAAe6 solid; padding:3px 3px 3px 3px; background-color:#FFFFFF;">
                    <a href="#" onclick="showdiv('matlogdiv')"  class=vmenu>Materiale- / udlægs -log</a>
	                </div>

                    <div id=Div1 style="position:absolute; visibility:visible; display:; top:-30px; width:100px; left:633px; border:1px #8cAAe6 solid; padding:3px 3px 3px 3px; background-color:#FFFFFF; z-index:2000000;">
                    <a href="erp_fakturaer_find.asp" class=vmenu target="_blank">Søg i fakturaer</a>
	                </div>
	                ...
	                <div id=knap_jobbesk style="position:absolute; visibility:visible; display:; top:15px; width:105px; left:195px; border:1px #8cAAe6 solid; border-right:0px #8cAAe6 solid; border-left:0px #8cAAe6 solid; padding:5px 5px 5px 5px; background-color:#ffffff;">
                     <table cellspacing=0 cellpadding=0 border=0 width=100%>
                     <tr><td colspan=2 class=lille>Step 3</td></tr>
                     <tr>
                        <td><a href="#" onclick="showdiv('jobbesk')" class=vmenu>Job / Aftale<br />beskriv.</a></td>
                        <td style="padding:3px 2px 0px 4px;"><img src="../ill/pil_fak.gif" border=0 /> </td></tr></table>
	                </div>
	               
	                
	               
	                <div id=knap_aktdiv style="position:absolute; visibility:visible; display:; top:15px; width:80px; left:275px; border:1px #8cAAe6 solid; border-right:0px #8cAAe6 solid; border-left:0px #8cAAe6 solid; padding:5px 5px 5px 5px; background-color:#ffffff;">
                     <table cellspacing=0 cellpadding=0 border=0 width=100%>
                     <tr><td colspan=2 class=lille>Step 4</td></tr>
                     <tr>
                        <td><a href="#" onclick="showdiv('aktdiv')" id="menushowakt" class=vmenu>Faktura<br />linier</a>
                        </td>
                        <td style="padding:3px 2px 0px 4px;"><img src="../ill/pil_fak.gif" border=0 /> </td></tr></table>
	                </div>
	                ...
	                <div id=knap_matdiv style="position:absolute; visibility:visible; display:; top:15px; width:95px; left:355px; border:1px #8cAAe6 solid; border-right:0px #8cAAe6 solid; border-left:0px #8cAAe6 solid;  padding:5px 5px 5px 5px; background-color:#ffffff;">
                     <table cellspacing=0 cellpadding=0 border=0 width=100%>
                     <tr><td colspan=2 class=lille>Step 5</td></tr>
                     <tr>
                        <td><a href="#" onclick="showdiv('matdiv')" class=vmenu>Mat. / udlæg<br />fak. linier</a>
                        </td>
                        <td style="padding:3px 2px 0px 4px;"><img src="../ill/pil_fak.gif" border=0 /> </td></tr></table>
	                </div>
	                ...
	                <div id=knap_betdiv style="position:absolute; visibility:visible; display:; top:15px; width:112px; left:450px; border:1px #8cAAe6 solid; border-left:0px #8cAAe6 solid; padding:5px 5px 5px 5px; background-color:#ffffff;">
                     <table cellspacing=0 cellpadding=0 border=0 width=100%>
	    ...
	    <div id="genindlaes" style="position:absolute; top:210px; left:435px; z-index:2;">
    ...
    oRec.close%>
	                ...
	                <tr>
		                ...
		                if not oRec.EOF then 
			                ...
			                if lcase(strLand) <> "danmark" OR lto = "epi" OR lto = "epi_osl" OR lto = "epi_sta" OR lto = "dencker" OR lto = "epi_ab" then
			                strLand = oRec("land")
			                strLandShow = strLand
			                lndCHK = "CHECKED"
			                else
                       ...
                       <%=modtageradr%>
                            ...
                            <span style="font-size:9px; color:#999999; line-height:10px;">Manuelle rettelser til adr. kan ikke overføres til EAN fak.</span>
		               ...
		               <tr>
		               <td><input type="checkbox" name="FM_land_vis" id="FM_land_vis" value="on" <%=lndCHK %>>Vis land
		               <br />
		               <input type="checkbox" name="FM_cvr_vis" id="FM_cvr_vis" value="on" <%=intModCvrCHK %>>Vis CVR nr.</td>	
		                </tr>
		                  <tr>	
                        ...
                        '** eller hvis rediger
                        if func <> "red" AND (cint(usealtadr) = 0 OR cint(forvalgtFakadr) <> 0) OR (func = "red" AND cint(strAtt) <> 0) then
		                attCHK = "CHECKED"
		                else
		                attCHK = ""
		                end if
		                 %>
			                <td valign=top>
                                ...
                                <br /><b>Benyt alternativ modtager adr.</b> (kontaktpers. / filial)<br />
                                <span style="font-size:9px; color:#999999;">Forvalgt fakturaadresse blandt kontaktpersoner/filialer.</span><br /> 
                                <input type="hidden" value="<%=func %>" id="altadrFunc" />
                                <input id="FM_usealtadr" name="FM_usealtadr" type="checkbox" value="1" <%=usealtadrCHK %> /> Ja 
                                <%if func = "red" then %> / Opdater <span style="font-size:9px; color:#999999;">(klik til/fra for at gen-indlæse adresse)</span> 
                                <%end if %><br />
                                ...
                                <a href="#" onclick="Javascript:window.open('kontaktpers.asp?menu=kund&kid=<%=kid%>&func=opr&rdir=fak', '', 'width=604,height=600,resizable=yes,scrollbars=yes')" class="rmenu">+ Opret ny</a> 
                                ...
                                <input type="checkbox" name="FM_att_vis" id="FM_att_vis" value="on" <%=attCHK %>><b>Vis Att. person</b> (vises først på fakturalayout)&nbsp;
	...
	<tr>
		...
		<tr>
		    ...
		    if ( oRec("mid") = cint(jobans1) AND func <> "red" AND (lto = "epi" OR lto = "epi_osl" OR lto = "epi_ab" OR lto = "epi_sta") _
            OR ( oRec("mid") = cint(session("mid") ) AND func <> "red" AND (lto <> "epi" AND lto <> "epi_osl" AND lto <> "epi_ab" AND lto <> "epi_sta" AND lto <> "synergi1") OR (vorref = oRec("mnavn") AND func = "red"))) _
		...
		<tr>
			<td><br /><br /><b>Vis afsender felter tilvalg:</b><br /><input type="checkbox" name="FM_ytlf_vis" value="on" <%=visAfsTlfCHK %>>Telefon nr.</td>
		</tr>
		<tr>
			<td><input type="checkbox" name="FM_yfax_vis" value="on" <%=visAfsFaxCHK %>>Fax</td>
		</tr>
		<tr>
		    <td><input type="checkbox" name="FM_yemail_vis" value="on" <%=visafsemailCHK %>>Email</td>
		</tr>
	
       

		<tr>
			<td><input type="checkbox" name="FM_yswift_vis" value="on" <%=visAfsSwiftCHK %>>Swift</td>
		</tr>
		<tr>
			<td><input type="checkbox" name="FM_yiban_vis" value="on" <%=visAfsIbanCHK %>>Iban</td>
		</tr>
		<tr>
			<td><input type="checkbox" name="FM_ycvr_vis" value="on" <%=visAfsCvrCHK %>>CVR nr.</td>
		</tr>
        <tr>
        <%
        strSQLKonto = "SELECT kid, bank, regnr, kontonr, bank_b, regnr_b, kontonr_b, bank_c, regnr_c, kontonr_c, bank_d, regnr_d, kontonr_d, bank_e, regnr_e, kontonr_e FROM kunder WHERE kid = "& afskid 'Selskab, licensejer eller datter selskab **'
		oRec.open strSQLKonto, oConn, 3 
		if not oRec.EOF then
        ...
        kontonr_sel0 = "SELECTED"
         ...
         %>
			...
			<td><br />Bankkonto: <select name="FM_afs_bankkonto" id="FM_afs_bankkonto" style="width:250px;">
	...
	<!-- afsender slut -->
            ...
            %>
            Hvis du ændrer <b>faktura systemdato</b>, følger fakturadatoen ikke længere det valgte datointerval og timer kan gå tabt, eller blive faktureret dobbelt.
            <br /><br />
            Benyt evt. <b>Labeldato istedet</b>
		    <br /><br />Faktura systemdatoen der afgør hvor <b>skæringsdatoen</b> mellem timer på denne faktura og næste faktura skal ligge. 
		    <br />&nbsp;
	            </td></tr></table>
	            </div>
	...
	<tr>
		...
		<b>Fakturadato:</b> (systemdato)<br />
        <span style="color:#999999; font-size:9px;">Skæringsdato for lukning af periode.</span>
		...
		if func <> "red" then 'cint(usedt_ival) = 1 and
            select  case lto 
            case "epi", "epi_osl", "epi_sta", "epi_ab", "intranet - local"
		...
		<tr>
		<td valign=top style="padding:4px 5px 2px 5px;"><input id="Checkbox1" name="FM_visperiode" value="1" type="checkbox" <%=chkvp %> /><span>Vis <b>periode</b> på faktura:</span>
		 ...
		 <%if func <> "red" AND useLastFakdato = 1 then %>
		  ...
		  <%
		    ...
		    uTxt = "Periode <b>start-dato</b> er korrigeret til dagen efter seneste faktura dato på dette job."
	        uWdt = 175
	        call infoUnisportAB(uWdt, uTxt, 40, 460)
		...
		<br /> Brug labeldato som fakturadato på fakturaprint. <span style="color:#999999; font-size:9px; line-height:10px;">Hvis der vælges <b>"labeldato"</b> bliver omsætningen beregnet udfra denne dato.</span>
		...
		<b>Forfaldsdato:</b></td><td valign=top style="padding:2px 5px 0px 5px;">
	    <%
        ...
        <%if func <> "red" AND (lto = "dencker" OR lto = "intranet - local") then
        klTjk = ""
        gkTjk = "CHECKED"
        else
        klTjk = "CHECKED"
		...
		<b>Status:</b></td><td valign=top style="padding:2px 5px 2px 5px;">
				<input type="radio" name="FM_betalt" value="0" <%=klTjk %>>Kladde&nbsp;&nbsp;&nbsp;
				<input type="radio" name="FM_betalt" value="1" <%=gkTjk %>><span style="color:yellowgreen;"><b>Godkendt</b></span>
		...
		<tr>
		<td style="padding:4px 5px 0px 5px;"><b>Rabatkolonne:</b>
         </td>
        ...
        <tr>
		<td style="padding:4px 5px 0px 5px;"><b>Antal- og enheds -kolonner:</b>
         </td>
		<td style="padding:4px 5px 0px 5px;">
           ...
           <%
                                ...
                                <input id="hideantenh" name="FM_hideantenh" type="checkbox" <%=hideantenhCHK %>>&nbsp;Skjul antal- og enheds -kolonner på faktura print. (ikke materiale linier)
        ...
        <%if intType <> 1 then '1 = kreditnota (En kreditnota kan IKKE være intern.) %>
         <tr>
		<td style="padding:12px 5px 20px 5px;" valign=top><b>Intern faktura:</b>
         </td>
		<td style="padding:8px 25px 20px 5px;" valign=top>
           ...
           <%
                                ...
                                Medregnes ikke i udfaktureret omsætning, ingen moms, egen nummerserie. Lukker periode/job for timeregistrering.
                                Interne fakturaer ignoreres ved beregning af timer til fakturering ved næste faktura oprettelse. Evt. posteringer i kontoplan foretages som normalt. 
                                </span>
		...
		<td style="padding:2px 5px 4px 5px;">
            ...
            sideskiftHSEL = ""
                ...
                if l <> 0 then
                    ...
                    case 31 
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (2 sider, tilføj manuelt ombrydning i brødteskt)</option>
                    <%
                    case 32 
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (3 sider, tilføj manuelt ombrydning i brødteskt)</option>
                    <%
                     case 33
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (4 sider, tilføj manuelt ombrydning i brødteskt)</option>
                     <%
                     case 34 
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (5 sider, tilføj manuelt ombrydning i brødteskt)</option>
                    <%
                    case 35 
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (6 sider, tilføj manuelt ombrydning i brødteskt)</option>
                    <%
                    case 36 
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (7 sider, tilføj manuelt ombrydning i brødteskt)</option>
                    <%
                    case 37 
                    %>
                    <option value="<%=l %>" <%=sideskiftHSEL %>> (8 sider, tilføj manuelt ombrydning i brødteskt)</option>
                     
                    <%case else%>
                    <option value="<%=l %>" <%=sideskiftHSEL %>><%=l %> linier</option>
                <%  end select
...
<div id="kontodiv" style="position:absolute; visibility:visible; width:720px; display:; top:640px; left:5px; border:1px #8cAAE6 solid; z-index:20000000000;">
 <table cellspacing=0 cellpadding=0 border=0 width=100% bgcolor="#ffffff">
 <tr>
	...
	<td valign=top style="padding:12px 5px 2px 20px;"><b>Debitor konto:</b><br />
		...
		fok = 0
				...
				oRec.open strSQL, oConn, 3 
				while not oRec.EOF 
				if (intModKonto = oRec("kid") AND func <> "red") OR (intModKonto = oRec("id") AND func = "red") then
				selkon = "SELECTED"
				fok = 1
				else
				selkon = ""
	...
	'	end if
		...
		<div id=fidiv_2 style="position:absolute; visibility:visible; display:; top:765px; width:720px; left:5px; border:0px #8cAAe6 solid;">
	...
	<tr>
		<td valign=top>
		<input type="checkbox" name="FM_visikkejobnavn" id="FM_visikkejobnavn" value="1" <%=visikkejobnavnCHK%>><span>Vis <b>ikke</b> job/aftale <b>navn</b> (overskrift) på faktura</span><br />
	
		<input type="checkbox" name="FM_visjobbesk" id="FM_visjobbesk" value="1" <%=visJb%>><span>Vis job/aftale <b>beskrivelse</b> på faktura</span>
    ...
    if jobstatus = 1 then
	    ...
	    %>
			...
			<br />
			<input type="checkbox" name="FM_lukjob" id="FM_lukjob" <%=lkjobCHK %> value="1" <%=diab %>><span>Luk job ved faktura oprettelse</span>
	    
	<br />
		
		
		<div style="position:relative; left:14px; top:10px; padding:10px; width:320px; height:250px; border:1px #CCCCCC solid; background-color:#F7F7F7;">
        <table cellpadding=0 cellspacing=1 border="0" width=100%>
        <tr><td class=lille>Jobnavn:</td><td class=lille><b><%=left(strJobnavn, 30) %> (<%=intjobnr %>)</b></td></tr>
		 <tr><td class=lille>Jobtype:</td><td class=lille><b><%=jType %></b></td></tr>
		...
		<tr><td class=lille>Status:</td><td class=lille><b><%=jobstatusTxt%></b> </td></tr>
		<tr><td class=lille>Bruttoomsætning:</td><td class=lille> <b><%=formatnumber(bruttooms, 2) &" "& valutaKode %></b> </td></tr>
		<tr><td class=lille>Timer forkalk:</td><td class=lille> <b><%=formatnumber(intBudgettimer, 2) %></b></td></tr>
        <tr><td class=lille>Kunderabat:</td><td class=lille> <b><%=intRabat %> %</b> </td></tr>
		...
		<tr><td class=lille>Periode:</td><td class=lille><b><%=formatdatetime(jobstdato, 2) %></b> til <b><%=formatdatetime(jobsldato, 2) %></b> </td></tr>
		<tr><td class=lille valign=top style="padding-top:5px;">Rekvnr/Indkøbsordre:</td><td class=lille><input type="text" name="FM_rekvnr" style="font-size:9px; width:150px;" value="<%=rekvnr %>" /> (opdaterer job) </td></tr>

        <%if aftaleId <> 0 then %>
        <tr><td class=lille>Aftale:</td><td class=lille> <b><%=aftalenavn %></b> </td></tr>
        <%end if %>
        ...
        <tr><td class=lille>SKI aftale </td><td class=lille><input id="FM_fak_ski" name="FM_fak_ski" type="checkbox" <%=skiCHK %> value="1" />  </td></tr>
        ...
        redim fl_antal(20), fl_besk(20), fl_vis(20), fl_enhpris(20), fl_valuta(20), fl_enhed(20), fl_rabat(20), fl_momsfri(20), fl_belob(20)
        
        call akttyper2009(2)
        
         %>
        <div style="position:relative; left:14px; top:10px; padding:10px; width:300px; height:250px; border:1px #CCCCCC solid; background-color:#F7F7F7;">
            <table cellpadding=0 cellspacing=1 border="0" width=100%>
            <tr><td class=lille>Aftalenavn:</td><td class=lille>
            <b><%=left(strNavn, 40) %></b></td></tr>
     
        <tr><td class=lille>Pris:</td><td class=lille> <b><%=formatnumber(intPris, 2) %></b></td></tr>
        <tr><td class=lille>Enheder:</td><td class=lille><b><%=formatnumber(intEnheder, 2) %></b></td></tr>
        <tr><td class=lille>Periode:</td><td class=lille><b><%=formatdatetime(startdato, 2) %></b> til  <b><%=formatdatetime(slutdato, 2) %></b></td></tr>
        <tr><td class=lille>Kunderabat:</td><td class=lille><b><%=intRabat %> %</b></td></tr> 
        <tr><td class=lille colspan=2>
        <b>Job tilknytte denne aftale:</b><br />
        <%strSQLaftalejob = "SELECT jobnavn, jobnr, id AS jid, valuta FROM job WHERE serviceaft = " & aftid
        ja = 0
        strJobPaaAft = ""
    ...
    <td style="padding-top:50px;">
       <a href="#" id="showjobtweet" class="vmenu">Job Tweet</a> | <a href="#" id="showinternnote" class="vmenu">Intern note</a>
        <br /><div id="internbesk_tweet" style="position:relative; width:300px; height:261px; overflow:auto; font-size:9px; border:0px #cccccc solid; padding:10px;"><b>Job Tweet:<br /></b><%=job_tweet %></div>
	...
	<tr><td colspan="2" style="padding:20px 20px 20px 20px;"><b>Beskrivelse:</b><br />
    <span style="font-size:11px; color:#999999;">Bemærk at sideombrydning er vejledende og kan forskubbe sig en smule ifht. tilvalg af rekv. nr. etc.<br>
    Editor-bredde er sat = faktura brødtekst bredde</span>
	...
	<tr>
		<td style="padding:0px 0px 0px 5px;">
               <b>Betalingsbetingelser / Kommentar:</b><br />
	 ...
	 <div id=betdiv_2 style="position:relative; visibility:hidden; display:none; top:80px; width:600px; left:5px; border:0px #8cAAe6 solid;">
        <table width=720 cellspacing=0 cellpadding=5 border=0><tr><td><a href="#" onclick="showdiv('aktdiv')" class=vmenu><< Forrige</a></td><td align=right>
...
<!--
    ...
    <%if fastpris = 1 then%>
    Fastpris: Brutto oms. fra budget.
    <%else %> 
    Lbn. timer: forbrug.
    <%end if %>
    ...
    <%end if %>
		...
		%>
        <br />
		<input type="text" id="FM_beloeb" name="FM_beloeb" style="width:80px; font-size:9px; border-bottom:2px #86B5E4 dashed;" value="<%=thistotbel%>"><span style="position:relative; width:20px; border-bottom:2px #86B5E4 dashed; padding:4px;" id="divbelobtot"><b><%=valutakodeSEL%></b></span>
