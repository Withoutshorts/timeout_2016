...
if len(session("user")) = 0 then
		...
		Response.end
			...
			'Response.flush
	            ...
	            if len(trim(request.form("FM_adviser_jobans"))) <> 0 then
                            ...
                            &" m1.mnavn AS m1mnavn, m1.email AS m1email,"_
				            &" m2.mnavn AS m2mnavn, m2.email AS m2email,"_
                            &" m3.mnavn AS m3mnavn, m3.email AS m3email,"_
                            &" m2.mnavn AS m4mnavn, m4.email AS m4email,"_    
                            &" m2.mnavn AS m5mnavn, m5.email AS m5email,"_    
				            ...
				            if request.servervariables("PATH_TRANSLATED") <> "C:\www\timeout_xp\wwwroot\ver2_1\timereg\jobs.asp" then
                                    ...
                                    if (jobans1 <> "0" AND isNULL(jobans1) <> true) OR (jobans2 <> "0" AND isNULL(jobans2) <> true) OR (jobans3 <> "0" AND isNULL(jobans3) <> true) OR (jobans4 <> "0" AND isNULL(jobans4) <> true) OR (jobans5 <> "0" AND isNULL(jobans5) <> true) then
		                            ...
		                            Mailer.AddRecipient "" & jobans2 & "", "" & jobans2email & ""
                                     ...
                                     if jobans3 <> "0" AND isNULL(jobans3) <> true then
		                                        Mailer.AddRecipient "" & jobans3 & "", "" & jobans3email & ""
            						            end if

                                    if jobans4 <> "0" AND isNULL(jobans4) <> true then
		                                    Mailer.AddRecipient "" & jobans4 & "", "" & jobans4email & ""
            						        end if

                                    if jobans5 <> "0" AND isNULL(jobans5) <> true then
		                                    Mailer.AddRecipient "" & jobans5 & "", "" & jobans5email & ""
            						        end if
    ...
    <%select case lto
    case "epi", "epi_cati", "epi_osl", "epi_sta", "epi_se", "intranet - local"
    %>
      <!-- Popup MSG -->
	...
	jo_gnstpris = 0
    select case lto 
    case "epi", "epi_osl", "epi_sta", "outz", "epi_ab", "epi_cati"
    jo_gnsfaktor = 2
	case else
    jo_gnsfaktor = 1
    end select
	...
	<!-- stam akt tilde SLUT --->
				   ...
				   <input id="jobfaktype" name="FM_jobfaktype" value="0" type="hidden" />   
					...
					<tr>
                        ...
                        if level = 1 then 
                             if lto <> "epi" OR (lto = "epi" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59 OR thisMid = 1 OR thisMid = 1720)) OR (lto = "epi_osl" AND thisMid = 2) OR (lto = "epi_ab" AND thisMid = 2) OR (lto = "epi_sta" AND thisMid = 2) OR (lto = "epi_cati" AND thisMid = 2) then%>
...
<%if editok = 1 then '*** rediger rettigheder ok%>	
	...
	<%if (lto = "epi" OR lto = "epi_osl" OR lto = "epi_sta" OR lto = "xintranet - local" OR lto = "epi_ab" OR lto = "epi_cati") AND func <> "red" AND step = 2 then 
   ...
   <tr><td colspan=2 style="font-size:9px; color:#CCCCCC;"><input type="radio" name="st_sel" value="0" id="st_cls_0" />Lukket</td></tr> 
    ...
    <%
                        ...
                        if level = 1 then
                             if lto <> "epi" OR (lto = "epi" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59 OR thisMid = 1720)) OR (lto = "epi_cati" AND (thisMid = 6 OR thisMid = 11 OR thisMid = 59)) OR (lto = "epi_osl" AND thisMid = 2) OR (lto = "epi_ab" AND thisMid = 2) OR (lto = "epi_sta" AND thisMid = 2) then 
		...
		<%
			    ...
			    while not oRec3.EOF 
                    ...
                    belobGrundVal = -valBelobBeregnet
                        ...
                        if cDate(oRec3("fakdato")) < cDate("01-06-2010") AND (lto = "epi" OR lto = "epi_osl" OR lto = "epi_sta" OR lto = "epi_ab" OR lto = "epi_cati") then
   ...
   <input type=image src="../ill/printer3.png" />
	...
	<%
        ...
        %><tr><td colspan="2"><br /><br />
            ...
            <a href="jobs.asp?menu=job&func=opret&id=0&int=1" class="alt">Opret nyt job  +</a>
                </div>
                <br />
            
            <%if lto = "outz" OR lto = "intranet - local" then %>
             <div style="background-color:forestgreen; padding:5px 5px 5px 5px; width:120px;">
            <a href="createJobFromFile.aspx?lto=<%=lto%>&editor=<%=session("user")%>" target="_blank" class="alt">Opret nyt job fra fil + </a>
             </div><br /><br />
                 <%end if
