...
if len(session("user")) = 0 then
	...
	<div id="topmenu" style="position:absolute; left:0; top:42px; visibility:visible;">
	<%call erpmainmenu(1)%>
	</div>
    ...
    call filterheader_2013(0,0,1090,oskrift)
    ...
    if len(request("FM_kunde")) <> 0 then
        ...
        if len(request.cookies("erp")("igndatoi")) <> 0 AND request.cookies("erp")("igndatoi") <> 0 then
         igndatoi = 1
        igndatoiCHK = "CHECKED"
        else
    ...
    response.cookies("erp")("igndatoi") = igndatoi
    ...
    <b>Kontakter:</b> <br>
		...
		<option value="0">Alle</option>
		    ...
		    <b>Søg på job nr.:</b> <input id="FM_sog" name="FM_sog" value="<%=sogval %>" type="text" style="width:305px; border:2px #6CAE1C solid;" /><br />
       ...
       jobStatChk1 = "CHECKED"
	    ...
	    response.Cookies("erp").expires = date + 90
        %>
        ...
        Ignorer datointerval, startdato = licens st.dato (<%=startDatoDag&"."&startDatoMd&"."&startDatoAar%>, dog maks. 2 år)<br /><br>
        ...
        <%
                  if print <> "j" AND media <> "print" AND media <> "eksport" then 
                    ...
                    if dateDiff("d", startDatoDag&"/"&startDatoMd&"/"&startDatoAar, strDag_slut&"/"&strMrd_slut&"/"&strAar_slut, 2,2) > 700 then

                    nyStDato = dateAdd("d", -700, strDag_slut&"/"&strMrd_slut&"/"&strAar_slut)

                    startDatoDag = day(nyStDato)
                    startDatoMd = month(nyStDato)
                    startDatoAar = right(year(nyStDato), 2)
        ...
        <input type="hidden" id="basis_listdato_dag" value="<%=startDatoDag%>"/>
        <input type="hidden" id="basis_listdato_mrd" value="<%=startDatoMd%>"/>
        <input type="hidden" id="basis_listdato_aar" value="<%="20"&right(startDatoAar, 2) %>"/>
    ...
    dim jobStatusVal, beloebFaktureret
    ...
    redim varMilepale(x), varMilepaleTxt(x), varTyperFak(x), jobStatusVal(x), beloebFaktureret(x), antalFak(x)
	...
	<tr bgcolor="#8cAAE6">
	    ...
	    <td class=alt valign=bottom style="padding:0px 10px 0px 0px; border-top:0px #ffffff solid;"><b>Seneste Faktura</b><span style="font-size:9px; color:#F7F7F7"><br />Systemdato<br />(ikke interne)</span><br />Faktureret ialt</td>
	    ...
	    <td valign=bottom bgcolor=#C4C4C4 style="padding:0px 10px 0px 0px; border-top:0px #ffffff solid;"><b>Seneste Faktura</b><span style="font-size:9px; color:#000000;"><br />Systemdato<br />(ikke interne)</span><br />Faktureret ialt</td>
	...
	<%end if %>
	<tr bgcolor="<%=varBG%>">
    <td bgcolor="#EFF3FF" valign="top" class="<%=jbclass%>"  style="border-bottom:1px #D6DFf5 solid; padding:3px 2px 2px 6px;"> 
   ...
   <td bgcolor="#EFF3FF" align=right valign=top style="border-bottom:1px #D6DFf5 solid; padding:3px 5px 2px 2px;">
	...
	<td bgcolor="#EFF3FF" align=right valign=top style="border-bottom:1px #D6DFf5 solid; border-right:1px #C4C4C4 solid; padding:3px 25px 0px 2px;">
	    ...
	    &nbsp;<a href="../timereg/erp_opr_faktura_fs.asp?FM_kunde=<%=kid(x)%>&FM_job=<%=jobid(x)%>&FM_aftale=0&FM_start_dag=<%=strDag%>&FM_start_mrd=<%=strMrd %>&FM_start_aar=<%=strAar%>&FM_slut_dag=<%=strDag_slut%>&FM_slut_mrd=<%=strMrd_slut%>&FM_slut_aar=<%=strAar_slut%>&reset=1&FM_usedatokri=1" class=todo_mellem><img
	...
	<td bgcolor="#FFFFFF" valign=top align=right style="border-bottom:1px #D6DFf5 solid; padding:5px 5px 0px 2px;">
        ...
        if varAftOrJob(x) = 1 then 'aftale
        %>
	    &nbsp;<a href="../timereg/erp_opr_faktura_fs.asp?FM_kunde=<%=kid(x)%>&FM_job=0&FM_aftale=<%=aftId(x)%>&FM_start_dag=<%=strDag%>&FM_start_mrd=<%=strMrd %>&FM_start_aar=<%=strAar%>&FM_slut_dag=<%=strDag_slut%>&FM_slut_mrd=<%=strMrd_slut%>&FM_slut_aar=<%=strAar_slut%>&reset=1&FM_usedatokri=1" class=todo_mellem><img
            src="../ill/ac0010-16.gif" border="0" alt="Opret faktura" /> </a>
		<%end if%>
		<%end if%>
		&nbsp;</td>
