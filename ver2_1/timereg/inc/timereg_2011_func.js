...
$(document).ready(function () {
    ...
    function filterOnOff(thisval) {
        ...
        if ($("#t_" + thisval).css('display') == "none") {
            ...
            $("#a_" + thisval).html("<img src='ill/minus2.gif' border=0> Tilføj job til aktiv-jobliste");
            ...
            $("#a_" + thisval).html("<img src=ill/plus.gif border=0> Tilføj job til aktiv-jobliste");
    ...
    if ($.cookie('showfilter') != "0") {
        ...
        $("#a_filterTreg").html("<img src='ill/minus2.gif' border=0> Tilføj job til aktiv-jobliste");

    } else {
        $("#t_filterTreg").hide(200);
        $("#a_filterTreg").html("<img src=ill/plus.gif border=0> Tilføj job til aktiv-jobliste");
    }
    ...
    function aktlist(thisval, srcl) {
        ...
        // tilføj linej ved bladet visning //
        ...
        $.post("?usemrn=" + usemrn + "&jobid=" + thisval + "&FM_easyreg=" + intEasyreg + "&FM_hr=" + intHR + "&ingTlaas=" + ingTlaas + "&ignakttype=" + ignakttype + "&job_aktids=" + job_aktids + "&lastFakDato=" + lastFakDato + "&job_akt_faser=" + job_akt_faser + "&job_iRowLoops=" + job_iRowLoops + "&stDato=" + stDato + "&slDato=" + slDato + "&FM_vistimereltid=" + vistimereltid + "&FM_ignJobogAktper=" + ignJobogAktper + "&FM_visSimpelAktLinje="+ visSimpelAktLinje +"&FM_jobstatus=" + lastJobstatus + "&sortByval=" + sortByval, { control: "FN_showakt", AjaxUpdateField: "true" }, function (data) {
            ...
            if ($("#FM_easyreg").is(':checked') == true || sortByval == 5 || $("#sortByval").val() == 6) { // Easyreg eller blandet liste
                ...
                if (antalAkt <= 40) {
                    var dvheigt = (antalAkt / 1 * rowhgt / 1) + 550
                } else {
                ...
                if (antalAkt <= 40) {
                    var dvheigt = (antalAkt / 1 * 38) + 160
                    $("#div_timereg_" + thisval).css("height", dvheigt);
                } else {
                    $("#div_timereg_" + thisval).css("height", 2000);
                }
