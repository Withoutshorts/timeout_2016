...
<%
	...
	Sortering på faktura: <input type="text" name="aktsort_<%=x%>" id="aktsort_<%=x%>" value="<%=thisaktsort(x)%>" style="font-size:9px; width:30px;"><br /> 
	...
	<%
		...
		'if instr(aktiswritten, "#"&thisaktid(x)&"#") = 0 then
					...
					'*** Beregning af enh pris og beløb ***'
	...
	Ved <b>fastpris</b> beregnes enhedspris udfra pris og antal angivet på aktiviteten.<br /><br />
	Ved <b>lbn. timer</b> beregnes enhedspris udfra (timepris på medarbejdere * real. timer) / antal der skal produceres angivet på akt.
