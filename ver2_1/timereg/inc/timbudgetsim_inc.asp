    ...
    %>
        ...
        for p = 0 to phigh - 1
            ...
            <input type="hidden" name="FM_medid" value="<%=antalm(m,1) %>" />
             ...
             <td class="afd_p_<%=p%> afd_p" style="visibility:<%=pvzb%>; display:<%=pdsp%>;"><input type="text" value="<%=formatnumber(medarbTp, 0) %>" style="width:40px;" /></td>
            ...
            <td class="afd_p_<%=p%> afd_p" style="white-space:nowrap; visibility:<%=pvzb%>; display:<%=pdsp%>;">= <input type="text" name="FM_H1H2" id="mh12h_jobaktmid_<%=jobid%>_<%=aktid %>_<%=antalm(m,1) %>" class="fs_<%=jobid%>_<%=aktid %>_<%=p %>" style="width:40px; background-color:#EFF3FF; border:0;" value="<%=h1h2%>" disabled /></td>
