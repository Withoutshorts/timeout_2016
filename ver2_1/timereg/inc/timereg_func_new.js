...
//function showdivakt(thisHTML, rloop){ 
//alert(rloop)
//obsampletxt = document.getElementById("Menu"+rloop);
//strHTML = "<table width='100%' border='0' cellspacing='1' cellpadding='0' bgcolor='#ffffff'>" + thisHTML + "</table>";
//alert(strHTML)
...
//} 
		...
		//function specialrp(iRowLoop, val, mlength, dtype) { 
		//if (val > 0) {
		//val = val
		//}else{
		//val = ""
		...
		//txt = "<input type='text' style='width:35; font-size:9px;' value='"+val+"' name='Timer_"+dtype+"_"+iRowLoop+"' id='Timer_"+dtype+"_"+iRowLoop+"' maxlength='"+mlength+"'>" //onKeyUp='setTimerTot("+iRowLoop+", 'man')', tjektimer("+dtype+","+iRowLoop+")
		//strHTML = txt + "&nbsp;<a href='#' onclick=expandkomm('man','"+iRowLoop+"');>+</a>" 
		//document.getElementById("specialtest_"+dtype+"_"+iRowLoop+"").innerHTML = strHTML
		//document.getElementById("Timer_"+dtype+"_"+iRowLoop+"").focus();
		//}
...
return true
	...
	//Aktiviteter expand
 	if (document.images){
		...
		function expand(thisHTML,de){
				//alert(strHTML)
				obsampletxt = document.getElementById("Menu" + de);
				strHTML = "<table width='100%' border='0' cellspacing='1' cellpadding='0' bgcolor='#ffffff'>" + thisHTML + "</table>";
				obsampletxt.insertAdjacentHTML("BeforeEnd", strHTML);
				//document.getElementById("Menu"+rloop).style.display = "";
