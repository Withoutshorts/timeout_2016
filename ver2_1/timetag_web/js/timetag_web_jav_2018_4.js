...
$(document).ready(function() {
   ...
   // $("#dv_akt").change(function () {
    ...
    function sogfil(jq_jobid) {

        $("#dv_fil").html('');
        
        $.post("?id=" + jq_jobid, { control: "FN_sogfil", AjaxUpdateField: "true" }, function (data) {
        ...
        if (jq_newfilterval.length > 0 || mobil_week_reg_job_dd == "1") {
            ...
            $.post("?jq_newfilterval=" + jq_newfilterval + "&jq_medid=" + jq_medid + "&varTjDatoUS_man=" + varTjDatoUS_man + "&jq_lto=" + jq_lto + "&thisJobid=" + thisJobid, { control: "FN_sogjobogkunde", AjaxUpdateField: "true" }, function (data) {
                ...
                $(".span_job").bind('click', function () {
                    ...
                    if (thisfile == "expence" && jq_lto == "lm") {
                        GetKundeNavn(thisJobid)
                    }
    ...
    function GetKundeNavn(jobid) {

        $.post("?jobid=" + jobid, { control: "GetKundeNavn", AjaxUpdateField: "true" }, function (data) {
            $("#kundenavnholder").html(data)
        });
    ...
    function showPosition(position) {
        ...
        if ($("#jq_longitude").val() != 0) {
            ...
            $.post("?jq_longitude=" + jq_longitude + "&jq_latitude=" + jq_latitude, { control: "FN_geolocation", AjaxUpdateField: "true" }, function (data) {
