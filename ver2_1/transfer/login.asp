        ...
        <div class="form-group" style="display:<%=fieldDisplay%>;">
         ...
         <!--   <div class="checkbox pull-left">
            ...
            %>
                ...
                <%else %>
                <input type="submit" class="btn btn-success btn-block" value="Check ind >> "/>
                <%end if %>
		...
		if founduser = 1 then
		    ...
		    if stransatkunde = "1" then 'else kunde login
               ...
               call indlaesTillaeg(lto)
                ...
                if lto = "esn" AND (session("mid") = 1 OR session("mid") = 14) then
                ...
                if cint(stempelur_hideloginOn) = 1 then 'skriv ikke login, men åben komme/gå tom
                    ...
                    '*****  Slået fra på Cflow 20180801 da stempelur_hideloginOn = 1 pr. dd
			...
			if browstype_client = "ip" then
                ...
                'if lto <> "miele" then
                    ...
                    if not oRec.EOF then
                        ...
                        case 10
                             response.redirect "https://timeout.cloud/timeout_xp/wwwroot/ver4_22/timetag_web/expence.asp"
                        case else
                            if lto <> "miele" AND lto <> "esn" then
                            response.redirect "https://timeout.cloud/timeout_xp/wwwroot/ver2_14/timetag_web/timetag_web.asp?flushsessionuser=1"
                            else
                            response.redirect "https://timeout.cloud/timeout_xp/wwwroot/ver4_22/timetag_web/checkin.asp"
                            end if
                        end select
