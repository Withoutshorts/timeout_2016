...
if len(session("user")) = 0 then
	...
	<tr bgcolor="#d6dff5">
		...
		<input type="text" name="FM_navn" value="<%=strNavn%>" style="width:700px;" maxlength=100><br />
		 (maks 100 karak. må ikke indeholde 'aprostrof eller "situations-tegn)</td>
	</tr>
	<tr bgcolor="#d6dff5">
		...
		Andre faser på dette job: (tilføj akt. til fase ved klik)<br />
		<%
		...
		Maks 100 tegn. Må ikke indeholde <b>.punktum, 'aprostrof, &-tegn eller mellemrum.</b></td>
	</tr>
	<%
	...
	<tr>
		...
		bliver alle de indtastede timer på denne aktivitet omsat med den angivne faktor. <br />
        Husk at indstille faktor efter den valgte type. Normalt vil det være naturligt at "ikke fakturerbar" står til faktor 0.
        </span>
        </td>
	</tr>
	...
	<tr bgcolor="#d6dff5">
		<td style="padding-left:40;" valign=top><br /><b>Forretningsområde(r):</b><br />
        <span style="font-size:10px; color:#999999;">Hvis forretningsområde ændres, <br />sker dette med tilbagevirkende kraft.</span></td>
	...
	<tr>
        ...
        <%if func <> "opretstam" then %>
        <a id="aktshowtp" href="aktiv.asp?menu=job&func=<%=func%>&id=<%=id%>&jobid=<%=jobid%>&jobnavn=<%=request("jobnavn")%>&rdir=<%=rdir %>&nomenu=<%=nomenu%>&showtp=<%=showtpVal%>&aktfavgp=<%=aktfavgp%>" class=vmenu><span style="color:green; font-size:16px;"><b>+</b></span> Rediger/Se medarbejder-timepriser på denne aktiviet (Reload)</a>
        <%end if %>
	...
	<tr>
		<td colspan=4><br>
		<img src="../ill/ac0001-24.gif" width="24" height="24" alt="" border="0">&nbsp;<b>Timepris på denne aktivitet.</b>
		<br>
		<%if func = "opretstam" OR func = "redstam" then%>
		Når denne aktivitet overføres til et job, kan de valgte timepriser f.eks bruges til at <br>
		fakturere den samme faste timepris for denne aktivitets type, hver gang aktiviteten overføres til et job.<br>
        
            
		
		<%else%>
	    Tildel timepriser på de medarbejderne der er tilknyttet dette job via deres projektgrupper.<br>
		...
		<script language="javascript">
			$(document).ready(function() {
				...
				(Angiv eller vælg)<br /><input type="button" class="checkAll" value="kolonne" /></td>
	...
	<%if aktfavgp <> "1" then %>
		...
		<font class=lillesort>Forkalkuleret timer og værdi på job:<br />
		...
		<br>
		Forkalkuleret timer og pris på aktive fakturerbare aktiviteter tilhørende dette job: (incl. denne)<br />
		Antal: <b><%=formatnumber(sumakttimer, 2) %></b> timer.<br />
		Pris: <b><%=formatnumber(sumaktbudget, 2) %></b> DKK</font>
        <br /><input id="opdjobv" name="opdjobv" value="1" type="checkbox" /> Sync. forkalkuleret timer og pris på job og aktiviteter. (incl. denne)
	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%" bgcolor="#EFF3FF" id="incidentlist">
 	<tr>
	    ...
	    <%
	    uWdt = 200
	    uTxt = "Træk i en stam-aktivitet for at sortere i rækkefølgen."
...
%>
	...
	<tr>
	    ...
	    <%
	    uWdt = 400
	    uTxt = "Træk i en aktivitet for at sortere i rækkefølgen.<br><b>Nb.:</b> Punktum, komma, &,#,¤,% tegn mfl. er ikke gyldige i <b>fasenavne</b> og vil blive udskiftet ved opdatering."
	    call infoUnisport(uWdt, uTxt) %>
	    </td>
	...
	<tr>
	    <td colspan=5 style="padding:20px 70px 20px 10px;">
            <input id="opdjobv" name="opdjobv" type="checkbox" <%=opdjobvCHK %> /><b>Synkroniser aktiviteter med job</b> (timer og omsætning)<br />
	...
	<br><br>
	<b>Funktioner:</b><br>
	<a href="aktiv.asp?menu=job&func=opdprogp&jobid=<%=jobid%>" class=vmenu>klik her</a> for at opdatere projektgrupper på alle ovenstående aktiviteter,<br> således at de matcher de projektgrupper der er tildelt på jobbet.
	    <!--
