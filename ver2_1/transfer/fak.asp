...
if len(session("user")) = 0 then
			...
			isInt = 0
					...
					isInt = 0
							...
							if session("erOprettet") <> "y" then
								...
								'***** n starter på 1 **************
								Response.write "<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>"
								'***** Aktivitets udspecificering ************
								for intcounter = 0 to antalAkt - 1
								
								thisAktId = request("aktId_n_"&intcounter&"")
								Response.write "thisAktId" & thisAktId & "<br>"
									...
									for intcounter3 = -(antalsumaktprakt) to antalsumaktprakt
										...
										if len(request("FM_show_akt_"& intcounter &"_"&intcounter3&"")) = 1 then
											...
											'***** Medarbejder udspecificering på print ************
											...
											for intcounter2 = 0 to antalmedspec - 1
													...
													if SQLBless2(thismedarbtpris) = SQLBless2(enhpris) then
														...
														'*** Skal medarbejder timer vises på print?
											...
											next 'Medarbejdere på print
									h = h + useh
									end if 'show sumaktivitet
													
													
										'***** Medarbejder udspecificering id db ************	
										...
										for intcounter2 = 0 to antalmedspec2 - 1
												...
												if SQLBless2(thismedarbtpris) = SQLBless2(enhpris) then
													...
													'if thisMid <> 0 AND len(thisAktId) <> 0 then
	...
	<tr>
		<td colspan=4 style="padding-left:10;" valign=top><br><b>Udspecificering.</b><br>
		...
		<%if request("faktype") = 0 then%>
			...
			<font class="megetlillesort">Når en faktura godkendes som endelig, bliver der oprettet en tilhørende postering på de ovenstående valgte konti.<br>
			Når fakturaen er godkendt kan den IKKE længere slettes.</font>
			<%end if%>
		<%else%>
	...
	<tr bgcolor="#5582D2">
		...
		<td colspan=4 style="border-top:1px #003399 solid;" class=alt><b>Faktura info.</b></td>
		<td align=right valign=top width="8"><img src="../ill/tabel_top_right.gif" width="8" height="23" alt="" border="0"></td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td style="border-left:1px #003399 solid;" rowspan="5">&nbsp;</td>
		<td colspan=4 height=5><img src="ill/blank.gif" width="1" height="1" alt="" border="0"></td>
		<td style="border-right:1px #003399 solid;" rowspan="5">&nbsp;</td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td colspan=4 style="padding-left:10;"><br><b>Job:</b><img src="ill/blank.gif" width="60" height="1" alt="" border="0">(<%=jobnr%>)&nbsp;&nbsp;<b><%=strJobnavn%></b></td>
	</tr>
	<tr bgcolor="#FFFFFF">
		<td colspan=4 style="padding-left:10;"><b><%=strFaktypeNavn%> nr:</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="text" name="FM_faknr" value="<%=strFaknr%>" size="14" maxlength="10">
		<input type="hidden" name="FM_faknr_opr" value="<%=strFaknr%>"><br>
		<%=strNote%><br>
	...
	if func <> "red" then%>
	<tr bgcolor="#FFFFFF">
		<td colspan=4 style="padding-left:10; padding-bottom:10;" valign=top><br><b>Periode afgrænsning:</b><br>
		Timer vist på denne faktura er alle indtastet i perioden:<br>
		Startdato: <b><%=formatdatetime(showStDato, 2)%></b>
		<%if useLastFakdato = 1 then%>
		&nbsp;(Dagen efter sidste fakturadato)&nbsp;
		<%end if%>
		<br>
		Slutdato:<img src="ill/blank.gif" width="5" height="1" alt="" border="0"> <b><%=formatdatetime(showSlutDato, 2)%></b></td>
	</tr>
	<%else%>
	<tr bgcolor="#FFFFFF">
	...
	if func = "red" then
		...
		While not oRec.EOF 
			...
			'Response.write thisaktid(x) &" n: "& thisaktnavn(x) &"<br>"
			 
			'Response.write thisaktnavn(x) & "<br>"
	...
	for x = 0 to x - 1 
		...
		sa = 0
					...
					<tr bgcolor="#66CC33">
						...
						<td style="border-top:1px #8caae6 solid; border-right:1px #8caae6 solid;">&nbsp;</td>
					</tr>
					...
					if func = "red" then
						...
						'strSQL10 = "SELECT DISTINCT(medarbejderid), tmnr, mnavn, mid, timeprisalt, medarbejdertype FROM progrupperelationer, timer"_
						'&" LEFT JOIN medarbejdere ON (mid = medarbejderid) "_  
						'&" LEFT JOIN timepriser ON (aktid = "& thisaktid(x) &" AND medarbid = medarbejderid) WHERE "_
						...
						'&" OR taktivitetid ="& thisaktid(x) &" AND tfaktim = 1 AND Tdato BETWEEN '" & stdatoKri &"' AND '"& slutdato &"' GROUP BY mid ORDER BY projektgruppeid"
					...
					while not oRec3.EOF
								...
								if func = "red" then
									...
									akt_showonfak = oRec3("akt_showonfak")
							...
							'**************************************************************************
							'Response.write thisaktid(x) &" # "& MedarbejderTimepris & " -- " & instr(medarbstrpris, "#"&MedarbejderTimepris&"#") & " -- " & x &"<br>"
							...
							<tr>
								...
								<input type="text" name="FM_m_fak_<%=n%>_<%=x%>" id="FM_m_fak_<%=n%>_<%=x%>" onKeyup="offsetfak('<%=x%>','<%=n%>'), tjektimer2('<%=x%>','<%=n%>')" value="<%=fak%>" size=4 style="!border: 1px; background-color: #FFFFFF; border-color: limegreen; border-style: solid; font-size:9px">
								...
								<td><input type="text" name="FM_m_tekst_<%=n%>_<%=x%>" value="<%=txt%>" size=45 style="!border: 1px; background-color: #FFFFFF; border-color: silver; border-style: solid; font-size:9px"></td>
			...
			<td colspan=3 bgcolor="#d6dff5" style="padding-left:5;"><font class="megetlillesort">ialt <%=thisaktnavn(x)%>:</font>
			<input type="text" name="timer_subtotal_akt_<%=x%>" id="timer_subtotal_akt_<%=x%>" value="<%=subtotaltimer%>" style='border:0px; font-size:9; width:45; background-color:#d6dff5;'><font class="megetlillesort"> timer</font></td>
			<td colspan=3 bgcolor="#d6dff5" align=right style="padding-right:5;">
			<input type="text" name="belob_subtotal_akt_<%=x%>" id="belob_subtotal_akt_<%=x%>" value="<%=subtotalbelob%>" style='border:0px; font-size:9; width:55; background-color:#d6dff5;'><font class="megetlillesort"> kr.</font></td>
			</tr>
			...
			<input type="hidden" name="highest_aval_<%=x%>" id="highest_aval_<%=x%>" value="<%=a%>">
			<%
		subtotaltimer = 0 
		subtotalbelob = 0 
	...
	%>&nbsp;&nbsp;&nbsp;<input type="text" name="FM_Timer" value="<%=intTimer%>" size="4" onkeyup="setTimerTot(<%=x-1%>)"; style="!border: 1px; background-color: #FFFFFF; border-color: limegreen; border-style: solid;">
	&nbsp;&nbsp;Timer ialt.&nbsp;</td>
	<td valign="top" align=right style="border-right:1px #8caae6 solid;"><img src="../ill/blank.gif" width="1" height="30" alt="" border="0"></td>
	</tr>
	<tr>
		<td valign="top" style="border-left:1px #8caae6 solid;"><img src="../ill/blank.gif" width="1" height="25" alt="" border="0"></td>
