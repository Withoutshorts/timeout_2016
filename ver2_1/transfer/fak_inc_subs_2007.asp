...
'*********************************************'
	    ...
	    if a = 0 then
				...
				useEkstraMedarbejderTimepris = ""
		            ...
		            if usefastpris <> 1 then
		                    ...
		                    useMedarbejderTimepris = ((mbelobtot)/thisAktForkalkStk(x)) 'thisTimePris(x)
    		                'MedarbejderTimepris * medarbejderTimer
    		                thisAkttimer(x) = 0 'thisAktTimer(x)
		                    thisAktBeloeb(x) = 0 'thisAktBeloeb(x)
    		                akt_showonfak = 0
				            aktrabat = 0
		                    
		                    else '*** Alm løbende timer
		                    useMedarbejderTimepris = -2
		                    thisAkttimer(x) = 0
		                    thisAktBeloeb(x) = 0
		                    akt_showonfak = 0
		            ...
		            '** fastpris ***
		                ...
		                '***************************************************************************************
		                '** Beregning mellem jobfastpris eller om det er aktiviteter der skaæ være grundlag
		                '** sker i job_inc_func linie 210 **'
		              
    		            useMedarbejderTimepris = thisTimePris(x) 'akttimepris 'jobFastprisTp 'thisTimePris(x)
    		            'thisAkttimer(x) = faktot 'thisAktTimer(x)
		...
		if thisaktfunc(x) = "red" then
			    ...
			    'if usefastpris <> 1 then 
			            ...
			            if a = 0 then
			                ...
			                '*** Fastpris / lbn timer ***'
			                if cint(usefastpris) <> 1 OR (thisAktForkalkStk(x) <> 0 AND thisAktBgr(x) = 2 AND cint(usefastpris) <> 1) then 
    			            '** Lbn. timer ***'
			                ...
			                '*** Fastoris ****'
			                        if cint(thisJoborAkt_tp(x)) = 0 then
			                            '** Job er grundlag på faspris '***
			                            valueTimer = 0
                                        'valueTimer = thisAktTimer(x)
					                    hiddenTimer = valueTimer
			        ...
			        'if len(triM(thisAktFase(x))) <> 0 then
			        'thistxt = replace(thisAktFase(x), "_", " ") &", "& thisAktText(x)
			        'else
			        thistxt = thisAktText(x)
			        'end if  
	...
	timepris = useMedarbejderTimepris '(akttimepris(x))
		...
		'**  dvs den blev fravalgt ved fakopr skal akt aldrig være sået til **'
		 if func = "red" AND thisaktfunc(x) <> "red" then
		    ...
		    if usefastpris = 1 AND func <> "red" then
			        ...
			        '*****************************************************************'
			            if hiddentimer <> 0 AND cint(thisaktCHK(x)) = 1 then
			            chk = "CHECKED"
			            else
			        ...
			        '*****************************************************************'
			                ...
			                '*** Porduktions løsning, beregnet samlet stk pris udfra timeforbrug (DENCKER) ****'
			                if (thisAktForkalkStk(x) <> 0 AND thisAktBgr(x) = 2) AND func <> "red" then
		...
		'**** Timer (antal) *****'
		if nul = 1 then '*** Sum akt ***'
		    strAktsubtotal = strAktsubtotal &"<input type=checkbox name='FM_show_akt_"&x&"_"&a&"' id='FM_show_akt_"&x&"_"&a&"' value='1' "&chk&" onClick='andreEnhprs("&x&","&a&"), setBeloebTot2("&x&")'></td>"
		...
		strAktsubtotal = strAktsubtotal &"<input type=checkbox name='FM_show_akt_"&x&"_"&a&"' id='FM_show_akt_"&x&"_"&a&"' value='1' "&chk&" onClick='setBeloebTot2("&x&"), andreEnhprs("&x&","&a&")'></td>"
			...
			& "<br><input type='button' name='beregn2_"&x&"_"&a&"_a' id='beregn2_"&x&"_"&a&"_a' value='Calc' style='font-size:8px' onClick='tjektimer("&x&","&a&"), andreEnhprs("&x&","&a&"), setBeloebThis2("&x&","&a&"), hideborder2("&x&","&a&")';></td>"
	...
	function fakturaMedarbLinjer(medlinieid)
	                            ...
	                            if thisaktfunc(x) = "red" AND medlinieid = 0 then
					                ...
					                valutaKodeKurs = oRec3("fmskurs")
										...
										'** For at undgå at der vises timer på akt. der ikkke er med på
							...
							<tr bgcolor="<%=mbg %>" id="tr_<%=n%>_<%=x%>" class="<%=trcls %>" style="visibility:<%=mVzb%>; display:<%=mDsp%>;">
								...
								<td valign=top><input type="text" maxlength="<%=maxl%>" name="FM_m_fak_<%=n%>_<%=x%>" id="FM_m_fak_<%=n%>_<%=x%>" onKeyup="offsetfak('<%=x%>','<%=n%>'), tjektimer2('<%=x%>','<%=n%>'), showborder('<%=x%>','<%=n%>')" value="<%=fak%>" style="border:1px yellowgreen solid; font-size:9px; width:30px;">&nbsp;<font class=lillegray><%=fakopr%></font>
								<input type="hidden" name="FM_m_fak_opr_<%=n%>_<%=x%>" id="FM_m_fak_opr_<%=n%>_<%=x%>" value="<%=fak%>" style="font-size:8px;">
								</td>
								
								<!-- Vente timer -->
								<td align=center valign=top style="padding:5px 0px 0px 0px;">
    ...
    %>
    <tr>
    <td colspan=12><b><%=oRec("matgrpnavn") %></b></td>
    ...
    <tr>
        ...
        <td>
            <input id="FM_matnavn_<%=m %>" name="FM_matnavn_<%=m %>" type="text" value="<%=replace(matnavn, chr(34), "''")  %>" style="font-family:verdana; font-size:9px; width:180px;" /></td>
        <td>
        ...
        <td> 
            ...
            <input id="FM_matenhedspris_<%=m %>" name="FM_matenhedspris_<%=m %>" type="text" value="<%=SQLBlessDOT(formatnumber(oRec("matsalgspris"), 2)) %>" onkeyup="offsetmatenhpris('<%=m %>'), tjekmatehpris('<%=m %>'), showbordermat('<%=m%>')" style="font-family:verdana; font-size:9px; width:60px;" /></td>
        <td align=center><%=valutaMatKode%>
        <input id="FM_matvaluta_<%=m %>" name="FM_matvaluta_<%=m %>" type="hidden" value="<%=valutaMatId%>" />
        </td>
        <td> <input id="FM_matenhed_<%=m %>" name="FM_matenhed_<%=m %>" type="text" value="<%=matEnhed%>" style="font-family:verdana; font-size:9px; width:40px;" /></td>
        <td>
        ...
        matSubTotal = ((oRec("matantal") * oRec("matsalgspris")) - (oRec("matantal") * oRec("matsalgspris") * (intMatRabat/100)))
        else
    ...
    <%if browstype_client <> "mz" then %>
     <div id="knap_godkend" style="position:absolute; visibility:visible; display:; top:47px; width:125px; height:53px; left:570px; border:1px #6CAE1C solid; padding:3px 5px 6px 5px; background-color:#DCF5BD;">
     <%else%>
     <div id="knap_godkend" style="position:absolute; visibility:visible; display:; top:47px; width:130px; height:47px; left:580px; border:1px #6CAE1C solid; padding:3px 5px 0px 5px; background-color:#DCF5BD;">
     <%end if%>
   ...
   %>
    ...
    <div style="position:absolute; left:725px; top:300px; padding:0px; width:200px; border:0px #CCCCCC solid;">
    <table width=100% border=0 cellspacing=0 cellpadding=0 bgcolor="#FFFFF1">
	<tr>
	    <td colspan=2 style="padding:10px 5px 2px 10px; border:1px #CCCCCC solid; border-bottom:0px;">
	        <b>Fakturerings valuta på faktura</b><br />
	        <%call selectAllValuta(2, jftp) %>
	   </td>
	</tr>
	<tr>
		<td colspan=2 style="padding:8px 5px 10px 10px; border:1px #CCCCCC solid; border-top:0px;"> 
		<b>Rabat</b> (Kun sum akt. linier)<br />
		                        ...
		                        %>
                                    <select id="FM_rabat_all" name="FM_rabat_all" style="width:50px; font-family:verdana; font-size:9px;" onChange="opd_rabatrall()">
	...
	chke1 = "CHECKED"
		...
		<br /><br /><b>Enheder</b> (globale indstillinger)<br />
		(Km. aktiviteter ændres ikke)<br />  
