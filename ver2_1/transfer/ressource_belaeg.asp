...
if len(session("user")) = 0 then
	...
	<img src="../ill/header_resbelaeg.gif" width="755" height="44" alt="" border="0">
	
	<br><br>
	<table border=0 cellspacing=0 cellpadding=0>
	<form action="ressource_belaeg.asp?menu=job" method="post" name="periode" id="periode">
	<tr>
	<td>
	...
	strSQL = "SELECT jobnavn, mnavn, job.id, job.jobnr, medarbejdertype, normtimer_son, normtimer_man, normtimer_tir, normtimer_ons, normtimer_tor, normtimer_fre, normtimer_lor, medarbejdere.mid, ressourcer.timer AS restimer, ressourcer.dato AS resdato FROM job, medarbejdere LEFT JOIN medarbejdertyper ON (medarbejdertyper.id = medarbejdertype) LEFT JOIN ressourcer ON (ressourcer.jobid = job.id "& resMedarbKri &" "& perKri &") WHERE "& restimJidKri &" "& restimMidKri &" AND fakturerbart = 1 ORDER BY medarbejdere.mid, job.jobnavn"
	oRec.open strSQL, oConn, 3 
	x = 0
	...
	if cint(intJid) <> 0 then
	jbTxt = "<a href='javascript:NewWin_large(""ressourcer.asp?menu=job&id="&lastJobID&""")' class=vmenu>"&lastJobnavn&"</a>" 
	else
...
for x = 0 to x - 1
	...
	%>
	<tr>
		<td valign=top><font class=megetlillesort>
		<div style="background-color:#eff3ff; border:1px #5582d2 solid; padding-left:2; width:76;"><a href="javascript:NewWin_todo('medarb_restimer.asp?menu=job&func=show&FM_medarb=<%=xmid(x)%>&FM_mednavn=<%=mnavn(x)%>')" class=megetlilleblaa><%=left(mnavn(x), 14)%></a></div>
		<div style="background-color:#8caae6; padding-left:2; width:76;">Realiseret</div>
		<div style="background-color:#ffffff; padding-left:2; width:76;">Tildelt</div>
		<div style="background-color:#ffffff; border:1px #ffffff solid; padding-left:1; width:76;">+ / - Balance</div>
		<div style="background-color:#8caae6; padding-left:2; width:76;">Norm./Real.</div>
		<div style="background-color:#ffffff; padding-left:2; width:76;">Norm./Tildelt</div>
		</td>
	<%end if
		...
		<%
				...
				weekthis = datepart("ww", nday)
				
				if weekthis <> lastweek AND y <> 0 AND lastxmid <> xmid(x) then
				%>
				<td valign=top width="28" align=center><font class=megetlillesort>
				...
				'Realiseret
					...
					rtimer0 = rtimerdato
						...
						Response.write "<div style='background-color:#ffffff; padding-left:2;'>"&showtimerpavalgtejob&"</div>"
				...
				<%if lastxmid <> xmid(x) then%>
				<td valign=top width="24" align=center><font class=megetlillesort>
				...
				<td valign=top width="30" align=center><font class=megetlillesort>
