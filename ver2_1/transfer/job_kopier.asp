...
if len(session("user")) = 0 then
	...
	case "dbkopier"
				...
				'Response.end
					...
					strDato = session("dato")
							...
							&" jobfaktype, rekvnr, "_
	                        &" jo_gnstpris, jo_gnsfaktor, jo_gnsbelob, jo_bruttofortj, jo_dbproc, udgifter) VALUES "_
							&"('"& strJobNavn &"', "_
							&""& newjobnr &", "_ 
							&""& trim(kids(x)) &", "_  
							&""& strBudget &", "_ 
							...
							while not oRec.EOF 
									...
									&" VALUES ("& varJobIdThis &", 0, "& oRec("medarbid") &", "& oRec("timeprisalt") &", "& oRec("6timepris") &")"
							...
							oRec.close 
	                                ...
	                                &" tidslaas_tor, tidslaas_fre, tidslaas_lor, tidslaas_son, fase "_
									...
									while not oRec.EOF 
										...
										strfase = oRec("fase")	
														...
														&""& tidslaas_man &", "& tidslaas_tir &", "& tidslaas_ons &", "_
						                                &""& tidslaas_tor &", "& tidslaas_fre &", "& tidslaas_lor &", "& tidslaas_son &", '"& strfase &"'"_
					                                    &")") 
							...
							oRec.close
				                ...
				                '*** Opdatere jobnrmaske ***'
				                strSQL = "UPDATE licens SET jobnr = "&  newjobnr &" WHERE id = 1"
				...
				if showaspopup <> "y" then '*** Jobsiden eller res-kalenderopr. siden
				    ...
				    %>
				    <script>
	                window.top.frames['a'].location.href = "timereg_akt_2006.asp?usemrn=<%=medarbejderid%>&showakt=0"
	                window.top.frames['t'].location.href = "timereg_2006.asp?FM_use_me=<%=medarbejderid%>"
	                </script>       
	                <%
				    case else
	...
	<input type="hidden" name="datoslkri" id="datoslkri" value="<%=request("datoslkri")%>">
		...
		<input type="checkbox" name="FM_ulev" id="FM_ulev" value="1" checked> Kopier underleverandører.<br>
