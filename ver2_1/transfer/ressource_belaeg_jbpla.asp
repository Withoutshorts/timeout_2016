...
<% 
	...
	&" LEFT JOIN job j ON (j.jobnr = t.tjobnr AND "& jobSQLkri &" AND j.jobnavn IS NOT NULL AND j.risiko > -1)"
    ...
    '***** Henter job der er med på aktivjoblisten for den enekelte medarbejder ****'
	...
	&" LEFT JOIN job j ON (j.id = tu.jobid AND j.risiko > -1)"
	
    strSQLtua = strSQLtua &" LEFT JOIN kunder k ON (kid = j.jobknr)"_
	&" LEFT JOIN medarbejdere m2 ON (m2.mid = j.jobans1)"_
	&" LEFT JOIN medarbejdere m3 ON (m3.mid = j.jobans2)"_
    &" WHERE medarb = "& mid &" "& jobMrealtimerPa & " AND forvalgt = 1 AND aktid = 0 AND "& jobSQLkri &" AND j.risiko > -1 AND jobstatus = 1"
...
if len(session("user")) = 0 then
    ...
    if len(trim(request("FM_vis_simpel"))) <> 0 AND request("FM_vis_simpel") <> 0 then
    vis_simpel = 1 'request("FM_vis_simpel")
    else
    vis_simpel = 0
    end if
    ...
    if cint(vis_simpel) = 0 then
    vis_simpelCHK = ""
    else
    vis_simpelCHK = "CHECKED"
    end if
    ...
    dim timerMJobTotal, realMJobTotal
	...
	Redim timerTotalTotal(2000), maanedY(2000), medarbTotalTimer(2000), medarbTimerReal(2000), realTotalTotal(2000) 
    Redim timerMJobTotal(2000), realMJobTotal(2000), fcGrandtotal(2000), realGrandtotal(2000), jobTimerGrandGrand(2000), jobBgtGrandGrand(2000)
    Redim procMJobTotal(2000), procTotalTotal(2000),  medarbTotalProc(2000), fpGrandtotal(2000) 
    ...
    jobStartKri = yearthis&"/"&monththis&"/"&stDatoDag
        ...
        jobStartKri = firstdayOfFirstWeek 'year(firstdayOfFirstWeek) &"-"& month(firstdayOfFirstWeek) &"-"& year(firstdayOfFirstWeek)
    ...
    jobSQLkri = ""
                ...
                if instr(jobSogVal, ",") <> 0 then '** Komma **'
                    ...
                    jobSQLkri = "(jobnavn LIKE '" & jobSogVal &"' OR jobnr LIKE '"& jobSogVal &"')"
                    end if
   ...
   response.cookies("resbel")("viskunjobiper") = viskunjobiper
													...
													if len(trim(tTimertildelt(y))) <> 0 then
															...
															if tTimertildelt(y) <> 0 then
																...
																timerThisMD = sqlbless(tTimertildelt(y))
                                                                    ...
                                                                    call normtimerPer(tmedid(y), nu, 6)
                                                                    procThisMD = replace(formatnumber(  (( (tTimertildelt(y)/nTimerPerIgnHellig)/weekFaktor)* 100), 0), ",",".")	
                                                                
                                                                else
									                            
                                                                procThisMD = sqlbless(tTimertildelt(y))	
                                                                
                                                                    call normtimerPer(tmedid(y), nu, 6)
                                                                    timerThisMD = replace(formatnumber((nTimerPerIgnHellig * weekFaktor * (tTimertildelt(y)/100)), 0), ",",".")						    
																...
																& " (jobid, medid, md, aar, timer, uge, aktid, proc, mdorweek) VALUES ("_
																&" "& tjobid(y) &", "& tmedid(y) &", "& monththis &", "& yearthis &", "& timerThisMD &", "& weekThis &", "& taktid(y) &", "& procThisMD &", "& mdorweek &")" 
	...
	<%
        ...
        <h4>Søg på jobnavn ell. nr.: <br /><span style="font-size:10px; font-weight:lighter;">(% wildcard, <b>231, 269</b> for specifikke job, <b>201-225</b> for interval)</span></h4>
               <input type="checkbox" value="1" id="FM_vis_job_fc_neg" name="FM_vis_job_fc_neg" <%=vis_job_fc_negCHK%> /> <b>Forecast overskreddet</b> <span style="color:#999999;">(kun job og aktiviteter, hvor forecast (timebudget) er overskreddet)</span>
                <br />
               <input type="checkbox" value="1" name="FM_vis_job_u_fc" <%=vis_job_u_fcCHK %> /> Vis aktive job <b>uden forecast</b> <span style="color:#999999;">(alle job på personlig aktivliste + job med realiserede timer i den valgte periode -5 md til dd.)</span> 
                <br />
            <input type="checkbox" value="1" id="FM_vis_simpel" name="FM_vis_simpel" <%=vis_simpelCHK %> /> Skjul <b>sub-totaler</b> på job og medarbejdere <span style="color:#999999;">(bedre overblik, hvis der er valgt mange medarbejdere) </span>  <br />
            
            <input type="checkbox" value="1" id="Checkbox1" name="FM_hideEmptyEmplyLines" <%=hideEmptyEmplyLinesCHK %> /> Skjul <b>tomme</b> medarbejderlinier <span style="color:#999999;">(bedre overblik, kan ikke tilføje forecast på nye medarbejdere) </span>   <br />        


             <input type="checkbox" value="1" id="Checkbox2" <%=vis_kolonne_simpelDisabled %> name="FM_vis_kolonne_simpel" <%=vis_kolonne_simpelCHK %> /> Skjul Ramme, Status og <input type="checkbox" value="1" id="Checkbox3" <%=vis_kolonne_simpel_aktDisabled %> name="FM_vis_kolonne_simpel_akt" <%=vis_kolonne_simpel_aktCHK %> /> Aktivitets kolonner.           
	...
	<%end if%>
      ...
      <br />
      <br /><b>Søg forecast på job:</b>&nbsp;<input id="sogtxt" name="FM_sog" value="<%=sogTxt %>" type="text" style="width:350px; font-size:14px; border:2px #6CAE1C solid;;" />
    ...
    <td valign=top style="padding-top:5px;">
        Angiv som <select name="FM_showasproc" onchange="submit();"><option value="0" <%=showasproc0Sel %>>Timer</option><option value="1" <%=showasproc1Sel %>>Procent</option></select></td>
	...
	strSQL = "SELECT jobnavn, m.mnavn, m.mnr, j.id AS jobid, j.jobnr, m.medarbejdertype, j.jo_bruttooms, j.jo_udgifter_ulev, j.stade_tim_proc, j.budgettimer, j.restestimat, "_
    &" m.mid, a.navn AS aktnavn, "
    strSQL = strSQL & "sum(rmd.timer) AS restimer, rmd.proc AS proc, rmd.md AS resmd, rmd.aar AS resaar, rmd.id AS resid, rmd.uge AS resuge, rmd.aktid AS aktid, rmd.mdorweek, "
    ...
    strSQL = strSQL &" GROUP BY m.mid, rmd.jobid, rmd.aktid, rmd.aar, rmd.md"
            ...
            if periodeSel = 3 OR cint(showasproc) = 1 then
            strSQL = strSQL &", rmd.uge"
            end if

    strSQL = strSQL &" ORDER BY m.mid, k.kkundenavn, rmd.jobid, a.fase, a.sortorder, a.navn" 'rmd.aktid
...
for x = 0 to antalxx - 1 'UBOUND(medarbKundeoplysX, 1) 'medarbKundeoplysX(x, 0)
	...
	csvTnormuge = ntimPer
        ...
        <td bgcolor="#ffffff" style="padding:5px 5px 5px 5px; white-space:nowrap;">
                        <%
                        ...
                        <%=medarbKundeoplysX(x, 5)%> (<%=medarbKundeoplysX(x, 6)%>)<br />

                        <b><%=medarbKundeoplysX(x, 2)%> (<%=medarbKundeoplysX(x, 1)%>)</b><br />
	    ...
	    forcastTimerJobtot = 0
					    ...
					    'Response.Write "<br>Nday: "& nday 
								    ...
								    '*** MD visning tjekker ikke om uge passer **'
								            ...
								            if (datepart("m", midjiddato(medarbKundeoplysX(x, 3),n, 3), 2,2) = datepart("m", nDay, 2,2)) AND _
								                ...
								                timerThis = midjiddato(medarbKundeoplysX(x, 3),n, 0)
                                                procThis = procThis + midjiddato(medarbKundeoplysX(x, 3),n, 6)
                                    ...
                                    if periodeSel <> 3 AND cint(medarbKundeoplysX(x, 24)) = 1 then 'fra uge til gns pr. md medarbKundeoplysX(x, 24) = 1 = ANGIVET SOM UGE
                                        
                                    'if divider = 0 then
                                    divider = dividerThisMth(datepart("m", nDay, 2,2))
                                    'end if 
				    ...
				    strSQLtimer = "SELECT sum(timer) AS sumtimerFaktisk FROM timer WHERE tmnr = "& medarbKundeoplysX(x, 3) &" AND tjobnr = '"&medarbKundeoplysX(x, 1)&"' "& aktSQLkri &" AND ("& aty_sql_realhours &") AND (YEAR(tdato) = "& datepart("yyyy", nday, 2, 2) &" AND " & mmwwSQLkri &")"
				    'Response.Write medarbKundeoplysX(x, 4) & " Sumtimer: "& strSQLtimer
				    'Response.Flush
				    'Response.end
				   ...
				   'Response.Write "sumTimer: " & sumTimer & "<br>"
					...
					<%if sumTimer <> 0 then %>
                        <br /><span style="font-size:8px;">
                         ...
                         <%if cint(showasproc) = 1 then %>
                                    <%=formatnumber(timerThis, 0) %> t. (<%=formatnumber(procThis, 0) %>%) <br />
                                <%end if%>

					<%=formatnumber(sumTimer, 0)%> t. ~ <%=formatnumber(afvThis, 0)%>%
                    ...
                    if (media = "eksport" AND vis <> 1)  then
                         ...
                         if lastxmid <> medarbKundeoplysX(x, 3) then
                            call normtimerPer(medarbKundeoplysX(x, 3), jobStartKri, 6) 
                    ...
                    <td bgcolor="#ffffff" valign=top class=lille align=right style="padding:2px 2px 2px 2px; white-space:nowrap;"><%=formatnumber(medarbKundeoplysX(x, 20), 0) %><br />
                       <span style="font-size:9px; color:#999999;"><%=formatnumber(medarbKundeoplysX(x, 19), 0) %> DKK<br />
...
*) Bemærk at realiserede timer kan medregnes dobbelt, hvis der er angivet forecast både uspecificeret og på en specifik aktivitet. Realiseret timer på denne aktivitet vil så indgå dobbelt i regnskabet.		
<br />
...
<%end if '** eksport %>
	 ...
	 <%end if
        ...
        if vis <> 1 then 'PIVOT
                     ...
                     if cint(visRamme) = 1 then 
                     csvTxtTop = csvTxtTop & "Normeret pr. uge;Ramme år;Ramme Timer;"
                     end if
           ...
           if media <> "print" AND media <> "eksport" AND media <> "chart" then
            ...
            <tr>
                <td align=center>
                <a href="#" onclick="Javascript:window.open('ressource_belaeg_jbpla.asp?media=eksport&<%=lnk%>', '', 'width=350,height=120,resizable=no,scrollbars=no')" class=rmenu><img src="../ill/export1.png" border=0></a>
                </td>
                
                </td><td>
                <a href="#" onclick="Javascript:window.open('ressource_belaeg_jbpla.asp?media=eksport&<%=lnk%>', '', 'width=350,height=120,resizable=no,scrollbars=no')" class=rmenu>.csv fil eksport (pivot)</a>
                </td>
                </tr>

                 <tr>
                <td align=center>
                <a href="#" onclick="Javascript:window.open('ressource_belaeg_jbpla.asp?media=eksport&<%=lnk%>&vis=1', '', 'width=350,height=120,resizable=no,scrollbars=no')" class=rmenu><img src="../ill/export1.png" border=0></a>
                </td>
                
                </td><td>
                <a href="#" onclick="Javascript:window.open('ressource_belaeg_jbpla.asp?media=eksport&<%=lnk%>&vis=1', '', 'width=350,height=120,resizable=no,scrollbars=no')" class=rmenu>.csv fil eksport</a>
                </td>
                </tr>
                
                <tr>
                <td align=center><a href="ressource_belaeg_jbpla.asp?media=print&<%=lnk%>" target="_blank"><img src="../ill/printer3.png" border="0"></a>
                                 
                             </td><td><a href="ressource_belaeg_jbpla.asp?media=print&<%=lnk%>" target="_blank" class=rmenu>Print version</a></td>
               </tr>
<!--
                  <tr>
