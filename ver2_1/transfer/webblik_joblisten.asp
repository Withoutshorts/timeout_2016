...
if len(session("user")) = 0 then
    ...
    <b>Job / kundeansvarlig -  vis kun job hvor: </b><br />
         ...
         <br />er 
        <input name="job_kans" id="job_kans" value="1" type="radio" <%=job_kans1CHK%> /> jobansvarlig (1-5)  <input name="job_kans" id="Radio1" value="2" type="radio" <%=job_kans2CHK%> /> kundeansvarlig 
   ...
   response.cookies("webblik")("viskunfs") = visKunFastpris
	...
	<tr height=30 bgcolor="#5582d2">
		...
		Job. ansv. & ejer<br />
         ...
         <td class=alt style="padding:3px;" valign=bottom><b><u>Forkalk. Budget</u> Forkalk. tim.<br /> Bruttooms. DKK</b><br />
        Salgsomk.<br />
        Intern kost.<br />
        <b>Budget DB %</b>
		</td>
		...
		<td class=alt style="padding:3px; width:80px;" valign=bottom><b><u>Real. timer</u></b><br /> Oms. DKK<br /> ~ timepris<br /><b>Intern kostpris real.</b><br /> ~ pr. time<br />(real timer * kostpris på medarb. type)
        </td>
        ...
        <td class=alt style="padding:3px;" valign=bottom><b><u>Rest. estimat</u></b><br />
		Balance og Afvigelse %<br />
       </td>

        <td class=alt style="padding:3px;" valign=bottom>Bal.</td>
		
        <td class=alt style="padding:3px;" valign=bottom><b><u>WIP</u> (Work In Progs. baseret på restestimat)</b><br />
        (Rest estimat * Brutto oms.)<br />
        Salgsomk. bdg.<br />
        Forv. intern kost.<br />
        <b>Forv. WIP DB %</b><br />
        (bdg. salgsomk. + forv. intern kost.<br /> / Bruttooms.)</td>

        <td class=alt style="padding:3px;" valign=bottom>Materiale frb. / udlæg DKK</td>
       
        <td class=alt style="padding:3px;" valign=bottom><b><u>Faktisk</u></b><br />
		<b>Faktureret DKK</b><br />
        (Fakt. udlæg/mat)<br />
		Salgsomk. real.<br />
        Intern kost. real.<br />
        <b>Faktisk DB %</b><br />
        ~ timepris <br />((faktureret - faktu. udlæg) / real. timer)
		</td>
		<td class=alt style="padding:3px;" valign=bottom><b><u>Balance DKK</u></b><br />
        (Faktisk - Brutto Oms.)<br />
        (Faktisk - WIP)</td>
	...
	&" risiko, udgifter, rekvnr, forventetslut, restestimat, jobstatus, j.kommentar, s.navn AS aftnavn, jo_dbproc, jo_bruttooms, jo_udgifter_intern, jo_udgifter_ulev, jo_gnsfaktor"_
	...
	<tr bgcolor="<%=bgthis%>">
        ...
        <tr>
            ...
            for dt = 0 TO 7
             ...
             dtnowHigh = dateadd("m", -7, now)
             end if

            

            %>
            <td align=center style="font-size:7px;"><%=left(monthname(month(dtnowHigh)), 1) %></td>
            <%next %>
         </tr>
         ...
         <tr bgcolor="#FFFFFF">
            ...
            %>
            <div style="height:<%=hgt%>px; width:15px; border-top:<%=bdThis%>px #CCCCCC solid; padding:0px; background-color:<%=bgThis%>;"><img src="../ill/blank.gif" width="15" height="<%=hgt%>" border="0" alt="<%=timerThis %>" /></div>
        ...
        <td valign=top class=lille style="padding:6px 3px 3px 3px; border-top:<%=btop%>px #cccccc dashed; border-right:1px #cccccc solid; white-space:nowrap;" align=right>
        <%="<b>"& formatnumber(timerTildelt, 2) & " t.</b>" %><br />
		<b><%=formatnumber(jobbudget, 2)%></b>
		...
		%>
		 <span style="font-size:9px; color:#999999; line-height:12px;">s.omk: </span><%=formatnumber(jo_udgifter_ulev, 2)%><br />
        <%=formatnumber(jo_udgifter_intern, 2)%><br />
        Budget DB: <b><%=formatnumber(oRec("jo_dbproc"),0) %> %</b>
		...
		<td valign=top style="padding:6px 3px 3px 3px; border-top:<%=btop%>px #cccccc dashed; border-right:1px #CCCCCC solid; border-left:1px #CCCCCC solid;" align=right class=lille><%=formatnumber(oRec("restimer"), 2)%> t.</td>
		
	
		

        <!-- Realiseret --> 
		<td valign=top style="padding:6px 3px 3px 3px; border-top:<%=btop%>px #cccccc dashed; white-space:nowrap; border-right:1px #cccccc solid;" align=right class=lille>
		<%
		...
		%>
		<b><%=formatnumber(timerforbrugt, 2)%> t.</b><br />
        
        <%=formatnumber(OmsReal, 2)%><br />
        ~ <%=formatnumber(tp, 2)%>
        
        
        <%OmsRealTot = OmsRealTot + OmsReal %><br />


		 <b><%=formatnumber(kostpris, 2)%></b><br />
         ...
         ~ <%=formatnumber(gnskostprtime, 2)%>
         <br />

         <%
         if OmsReal <> 0 AND kostpris <> 0 then


         if OmsReal >= kostpris then
         realDB = (100 - ((kostpris/OmsReal) * 100)) 
         else
         realDB = -((OmsReal/kostpris) * 100)
         end if
		...
		<td valign=top style="padding:6px 3px 3px 3px; white-space:nowrap; border-top:<%=btop%>px #cccccc dashed; border-right:1px #cccccc solid;" class=lille align=right>
		(<%=afsl_proc %> %) = <b><%=formatnumber(OmsWIP, 2) %></b><br />
         <span style="font-size:9px; color:#999999; line-height:12px;">s.omk: </span><%=formatnumber(jo_udgifter_ulev, 2) %><br />
        (<%=formatnumber(totalforbrugt, 0) %> t.) = <%=formatnumber(interKostEsti, 2) %><br />
        Forv. WIP DB: <b><%=formatnumber(forvDb, 0) %> %</b>
		</td>
        ...
        <td valign=top style="padding:6px 3px 3px 3px; border-top:<%=btop%>px #cccccc dashed; white-space:nowrap; border-right:1px #cccccc solid;" class=lille align=right><%=formatnumber(salgsOmkFaktisk,2) %></td>


        <td valign=top style="padding:6px 3px 3px 3px; border-top:<%=btop%>px #cccccc dashed; white-space:nowrap; border-right:1px #cccccc solid;" class=lille align=right>
		<b><%=formatnumber(faktureret)%></b> <br />
        (ekskl. mat og km:<br /> <%=formatnumber(faktureretTimerEnhStk,2) %>)<br />


         <span style="font-size:9px; color:#999999; line-height:12px;">s.omk: </span><%=formatnumber(salgsOmkFaktisk,2) %><br />
         <span style="font-size:9px; color:#999999; line-height:12px;">int. kost: </span><%=formatnumber(kostpris,2) %><br />
         Faktisk DB: <b><%=formatnumber(db2, 0)%> %</b><br />
         ~ <%=formatnumber(gnstimepris) %> pr/t.<br />
		...
		<b><%="<font color="& fcol &">" & formatnumber(bal, 2)%></font></b>
    ...
    <b><%=formatnumber(budgetIalt, 2)%></b><br />
	<span style="font-size:9px; color:#999999; line-height:12px;">s.omk: </span><%=formatnumber(udgifterTot, 2) %></td>
	<td style="border-top:1px #cccccc solid;">&nbsp;</td>
	<td valign=top align=right class=lille bgcolor="#c4c4c4" style="border-top:1px #999999 solid; border-left:1px #999999 solid; border-right:1px #999999 solid; padding:6px 3px 3px 3px;" nowrap><b><%=formatnumber(restimerIalt, 2)%> t.</b> </td>
	<td valign=top align=right class=lille style="border-top:1px #cccccc solid; border-right:1px #cccccc solid; white-space:nowrap; padding:6px 3px 3px 3px;"><%=formatnumber(timerforbrugtIalt, 2) %> t.<br />
    <%=formatnumber(OmsRealTot, 2) %></td>
    <td colspan=3 align=right class=lille style="border-top:1px #cccccc solid; border-right:1px #cccccc solid;">
        &nbsp;</td>
   
   <td align=right class=lille valign=top style="border-top:1px #cccccc solid; white-space:nowrap; border-right:1px #cccccc solid; padding:6px 3px 3px 3px;">
	<b><%=formatnumber(OmsWIPtot, 2) %></b></td>
   

	<td align=right valign=top class=lille style="border-top:1px #cccccc solid; white-space:nowrap; padding:6px 3px 3px 3px; border-right:1px #cccccc solid;"><%=formatnumber(salgsOmkFaktiskTot,2) %></td>
   
    <td align=right valign=top class=lille style="border-top:1px #cccccc solid; white-space:nowrap; padding:6px 3px 3px 3px;">
	<b><%=formatnumber(faktotbel, 2) %></b>
	</td>
	...
	sæt prioitet = -1
    <br /><br />
    <b>Sortering:</b><br>Træk i et job for at prioitere rækkefølgen. <br>Prioitets-angivelsen skifter først ved gen-indlæs.
	</td>
	</tr>
	</table>
	</div>
