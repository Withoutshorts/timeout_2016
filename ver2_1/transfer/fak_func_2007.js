...
$(document).ready(function () {
    ...
    $("#FM_fakdato").change(function () { if (IsValidDate($("#FM_fakdato").val()) == true) { var datestring = $(this).val(); var datesplit = datestring.split('.'); $("[name=FM_start_dag]").val(datesplit[0]); $("[name=FM_start_mrd]").val(datesplit[1]); $("[name=FM_start_aar]").val(datesplit[2]); GetBetIntVal(); } else { alert("dato ikke gemt"); } });
    ...
    function GetMatTilFak() {
        ...
        $.post("?jq_lto=" + jq_lto.val() + "&jq_inttype=" + jq_intType.val() + "&jq_id=" + jq_id.val() + "&jq_stdato=" + jq_stdato.val() + "&jq_sldato=" + jq_sldato.val() + "&jq_func=" + jq_func.val() + "&jq_valutaValkurs=" + valutaValkurs.val() + "&jq_valutafaklabel=" + valutaValLabel + "&jq_kuneks=" + viskuneks + "&jq_ignper=" + ignper + "&jq_jobid=" + jobid_val.val() + "&jq_aftid=" + aftid_val.val(), { control: "FN_getMatTilFak", AjaxUpdateField: "true", cust: kid_val.val() }, function (data) {
            ...
            $("#divbelobtot").html("<b>" + totbel + " " + valutaValLabel + "</b>");
            ...
            $("#divbelobtot_umoms").html("Beløb uden moms: <br> (" + totbel_umoms + " " + valutaValLabel + ") ");
    ...
    //if (document.getElementById("FM_job").value != 0) {
        GetMatTilFak();
    //}
    ...
    $(".qlinks").click(function () {
        ...
        if ($("#sumakt_" + thisid).css('display') == "none") {
            ...
            $.scrollTo("#sumakt_" + thisid, 1500, { axis: 'y' });
...
function setmTxt(aktid, n, x) {
    ...
    function opdatervalutAllelinier(a,x) {
		...
		if (kCode == 39){
				...
				////////////////////////////////
				        ...
				        //} else {
				         ...
				         //   belobakttot = belobsubakt
				         //   belobakttot_umoms = belobsubaktUmoms
				    ...
				    document.getElementById("divbelobtot").innerHTML = "<b>" + document.getElementById("FM_beloeb").value + " " + valutaKodeFak + "</b>"
				    document.getElementById("divtimerbelobtot").innerHTML = "<b>" + document.getElementById("FM_timer_beloeb").value + " " + valutaKodeFak + "</b>"
				    document.getElementById("divbelobtot_umoms").innerHTML = "Beløb uden moms: <br> (" + document.getElementById("FM_beloeb_umoms").value + " " + valutaKodeFak + ") "
	...
	var valutaKodeFak = document.getElementById("FM_valuta_all_1").options[document.getElementById("FM_valuta_all_1").selectedIndex].text
	            ...
	            document.getElementById("divbelobtot").innerHTML = "<b>"+belobialtHTML+" "+ valutaKodeFak +"</b>"
...
//opdvalAlleForkalk(1,x)
    ...
    function opdvalAlleForkalk(a,x,typ) {
