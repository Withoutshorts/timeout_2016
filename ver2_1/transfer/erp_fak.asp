...
case "FN_getMatTilFak"
	                        ...
	                        &" WHERE (mf.jobid = "& jobid & " OR mf.serviceaft = "& aftid &") "  
...
if len(session("user")) = 0 then
			...
			isInt = 0
					...
					isInt = 0
							...
							if func = "dbred" then
                                            ...
                                            &" hidefasesum = "& hidefasesum &", hideantenh = "& hideantenh &", medregnikkeioms = "& medregnikkeioms 
							...
							if func = "dbopr" then
                                                ...
                                                fak_laast = 0
                                                    ...
                                                    &" visikkejobnavn, hidefasesum, hideantenh, medregnikkeioms, fak_laast) VALUES ("_
													&" '"& intFaknum &"',"_
													&" '"& fakDato &"',"_
													&" "& jobid &","_
													&" "& intTimer &","_
                                                    ...
                                                    &" "& hidefasesum &", "& hideantenh &", "& medregnikkeioms &", "& fak_laast &")")
													
													'Response.Write "subtotaltilmoms: " & subtotaltilmoms & " intMoms: "& intMoms &"<br>"
													'Response.write strSQL & "<br><br>"
													'Response.flushwerwersdf sdf sdf sdf
													oConn.execute(strSQL)
	...
	'*********************************************************************
						...
						&" sideskiftlinier, labeldato, fak_abo, fak_ubv, visikkejobnavn, hidefasesum, hideantenh, medregnikkeioms, fak_laast"_
	                ...
	                if jobid <> 0 then
							            ...
							            &" ikkebudgettimer, jobans1, jobans2, rekvnr, jobstatus, usejoborakt_tp, fastpris, s.navn AS aftalenavn, aftalenr, serviceaft "_
                                        &" FROM job j LEFT JOIN serviceaft s ON (s.id = serviceaft) WHERE j.id = " & jobid
	...
	<%
		...
		&" jobstartdato, jobslutdato, jobfaktype, rekvnr, jobstatus, usejoborakt_tp, ski, abo, ubv, s.navn AS aftalenavn, aftalenr, serviceaft FROM job j "_
		...
		if cint(chktlffax) = 1 OR (lto = "epi") OR (lto = "intranet - local") then
		visAfsTlf = 1
		visAfsFax = 0
		else
		visAfsTlf = 0
	...
	%>
	              ...
	              <b>Beregner side, et øjeblik...</b><br /><br />
                      ...
                      Hvis der er valgt et langt dato interval, eller hvis der er mange medarbejder- og aktivitets -linier, kan det lidt tid at vise siden.
	                    ...
	                    <%
	                    antalaktCount = 0
	                    strSQL = "SELECT COUNT(id) AS antalaktCount FROM aktiviteter WHERE job = "& jobid &" AND aktstatus = 1 GROUP BY job" 
	                    ...
	                    Der er <b><%=antalaktCount%> aktive aktiviteter</b>  på dette job. Hver aktivitet tager 0,2-2 sek. at loade.
    ...
    oRec.close%>
	                ...
	                <tr>
		                ...
		                if not oRec.EOF then 
			                ...
			                if lcase(strLand) <> "danmark" OR lto = "epi" then
			                strLand =  "<br>"& oRec("land")
			                strLandShow = strLand
			                lndCHK = "CHECKED"
			                else
                			...
                			modtageradr = oRec("kkundenavn") & "<br>"& oRec("adresse") & "<br>"& oRec("postnr") & " " &oRec("city") & strLand
		                ...
		                attCHK = ""
		                 ...
		                 %>
			                <td>
                                ...
                                <a href="#" onclick="Javascript:window.open('kontaktpers.asp?menu=kund&kid=<%=kid%>&func=opr&rdir=fak', '', 'width=604,height=600,resizable=yes,scrollbars=yes')" class="rmenu" target="_blank">+ Opret ny</a> 
				                </td>
				                </tr>
				                <tr>
				                <td>
	...
	<tr>
		...
		<tr>
		    ...
		    if (oRec("mid") = cint(jobans1) AND func <> "red" AND lto = "epi") _
            OR (oRec("mid") = cint(session("mid")) AND func <> "red" AND lto <> "epi") OR (vorref = oRec("mnavn") AND func = "red") _
            OR (lto = "jttek" AND func = "red") then
            vfSEL = "SELECTED"
		    else
		    vfSEL = ""
		    end if  
	...
	<tr>
		...
		<br /> Brug labeldato som fakturadato på fakturaprint.<br />
        <span style="color:#999999; font-size:9px;">Hvis der vælges <b>"labeldato"</b> bliver omsætningen beregnet udfra denne dato.</span>
		...
		<b>Status:</b></td><td valign=top style="padding:2px 5px 2px 5px;">
				...
				<input type="radio" name="FM_betalt" value="1"><span style="color:yellowgreen;"><b>Godkendt</b></span>
        ...
        <%if intType <> 1 then '1 = kreditnota (En kreditnota kan IKKE være intern.) %>
         <tr>
		<td style="padding:8px 5px 0px 5px; background-color:#FFFFe1;" valign=top><b>Intern faktura:</b>
         </td>
		<td style="padding:4px 5px 0px 5px; background-color:#FFFFe1;" valign=top>
...
<div id="kontodiv" style="position:absolute; visibility:visible; width:720px; display:; top:570px; left:5px; border:1px #8cAAE6 solid;">
 <table cellspacing=0 cellpadding=0 border=0 width=100% bgcolor="#ffffff">
 <tr>
	...
	'	end if
		...
		<div id=fidiv_2 style="position:absolute; visibility:visible; display:; top:725px; width:720px; left:5px; border:0px #8cAAe6 solid;">
	...
	<tr>
		<td valign=top>
		<input type="checkbox" name="FM_visikkejobnavn" id="FM_visikkejobnavn" value="1" <%=visikkejobnavnCHK%>>Vis <b>ikke</b> job/aftale <b>navn</b> (overskrift) på faktura<br />
	
		<input type="checkbox" name="FM_visjobbesk" id="FM_visjobbesk" value="1" <%=visJb%>>Vis job/aftale <b>beskrivelse</b> på faktura
	...
	<br />
        ...
        <b><%=strNavn %></b>
        <br />
        Pris: <b><%=formatnumber(intPris, 2) %></b> <br />
        Enheder: <b><%=formatnumber(intEnheder, 2) %></b><br />
        Periode: <b><%=formatdatetime(startdato, 2) %></b> til  <b><%=formatdatetime(slutdato, 2) %></b>
        <br /><br />
        <b>Job tilknytte denne aftale:</b><br />
        <%strSQLaftalejob = "SELECT jobnavn, jobnr FROM job WHERE serviceaft = " & aftid
        ja = 0
        strJobPaaAft = ""
	...
	<tr><td style="padding:20px 20px 20px 20px;"><b>Beskrivelse:</b><br />
	...
	<div id=jobbesk_2 style="position:absolute; visibility:hidden; display:none; top:666px; width:720px; left:5px; border:0px #8cAAe6 solid;">
