...
if len(session("user")) = 0 then
	...
	'*** Her indsættes en ny type i db ****
		...
		if len(request("FM_prio_typ")) <> 0 then
		prio_typ = request("FM_prio_typ")
		else
		prio_typ = 1
		end if
		...
		&" "& dblfaktor &", "& intkunweekend &", "& intadvisering &", "& prio_grp &", "& prio_typ &")")
		...
		&" kunweekend = "& intkunweekend &", advisering = "& intadvisering &", priogrp = "& prio_grp &", type = "& prio_typ &" WHERE id = "&id&"")
	...
	<input type="hidden" name="id" value="<%=id%>">
    	<td valign="top" colspan="2"><font class="pageheader">Kontrolpanel | Incident prioitet | <%=varbroedkrumme%></font></td>
	</tr>
	...
	<tr>
		<td style="padding-top:10px;"><b>Responstid:</b></td>
	...
	<tr>
		<td style="padding-top:10px;"><b>Respons kun på arbejdsdage:</b><br>
		Responstid dækker ikke weekend og helligdage.</td>
	...
	<tr>
		<td style="padding-top:10px;"><b>Type:</b><br>
		Bruges udelukkende til at sortere efter på <br>Incidentlisten, hvis der ikke er valgt en kontakt.</td><td style="padding-top:10px;"><select name="FM_prio_typ" id="FM_prio_typ">
	<%
	strSQL = "SELECT id, navn FROM sdsk_prio_typ WHERE id <> 0"
	oRec.open strSQL, oConn, 3
	while not oRec.EOF
		if cint(prio_typ) = oRec("id") then
		tpSEL = "SELECTED"
		else
		tpSel = ""
		end if%>
		
	<option value="<%=oRec("id")%>" <%=tpSel%>><%=oRec("navn")%></option>
	<%
	oRec.movenext
	wend
	...
	<table cellspacing="0" cellpadding="0" border="0" width="600">
	<tr>
    <td valign="top"><font class="pageheader">Kontrolpanel | Incident prioitet</font>
	<br>
	<a href="sdsk_prio_grp.asp">Til gruppeoversigt...</a>
	<img src="../ill/blank.gif" width="350" height="1" alt="" border="0">
	<a href="sdsk_prioitet.asp?menu=tok&func=opret&prio_grp=<%=prio_grp%>">Opret ny prioitet <img src="../ill/pillillexp_tp.gif" width="16" height="18" alt="" border="0"></a><br>
	<br>&nbsp;
	</td></tr>
	</table>
	...
	Vælg gruppe: &nbsp;<select name="FM_prio_grp" id="FM_prio_grp">
	<%
	...
	<tr bgcolor="#5582D2">
		...
		<td width="300" class=alt><b>Prioitet</b> (Antal inci.)</td>
