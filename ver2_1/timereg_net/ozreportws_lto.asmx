...
Public Class ozreportws_lto
    ...
    Public Function oz_report_lto(ByVal ds As DataSet) As String
        ...
        Dim strSQLadmin As String = "SELECT email, navn, lto, rapporttype, medid, medarbejdertyper, projektgrupper FROM rapport_abo WHERE id <> 0 AND (rapporttype = 1 OR rapporttype = 0 OR rapporttype = 2 OR rapporttype = 3) " & strLtoKri & " " & EmailToKri & " ORDER BY id " & lto_limitkri
        ...
        While objDR_admin.Read() = True
            ...
            Select Case CInt(rapporttype)
                ...
                Case 2, 3 'Rapporttype 2 og 3 Rapport 2 = projektoverblik for jobansvarlige Rapport = 3 Kun egne projekter
                    ...
                    Using writer As StreamWriter = New StreamWriter("D:\webserver\wwwroot\timeout_xp\wwwroot\ver4_22\inc\upload\" & lto & "\" & flname2, False, Encoding.GetEncoding("iso-8859-1"))
                ...
                Case 1 'Rapporttype 4 - ressource forecast rapport SKAL VÆRE 4
                    ...
                    Using writer As StreamWriter = New StreamWriter("D:\webserver\wwwroot\timeout_xp\wwwroot\ver4_22\inc\upload\" & lto & "\" & flname4, False, Encoding.GetEncoding("iso-8859-1"))

                        ExpTxtheader = "Job;Jobnr;Aktivitet;"
                        expTxt = ""

                        Dim strSQLextmedarb As String = "SELECT jobid, medid, m.mnavn as mnavn, m.init as medarbinit FROM ressourcer_md as r LEFT JOIN job as j ON (j.id = r.jobid) LEFT JOIN medarbejdere as m ON (m.mid = r.medid) WHERE j.jobstatus = 1 GROUP BY medid"
                        ...
                        Dim strSQLextjob1 As String = "SELECT jobid, medid, m.mnavn as mnavn, j.jobnavn as jobnavn, aktid, j.jobnr as jobnr FROM ressourcer_md as r LEFT JOIN job as j ON (j.id = r.jobid) LEFT JOIN medarbejdere as m ON (m.mid = r.medid) WHERE j.jobstatus = 1 GROUP BY jobid, medid"
                        ...
                        While objDR.Read()
                            ...
                            If lastjobid <> objDR.Item("jobid") Then
                                ...
                                While objDR2.Read()
                                    ...
                                    Dim strSQLextJobMedarbTimer As String = "SELECT sum(timer) as timer FROM ressourcer_md WHERE jobid = " & objDR.Item("jobid") & " AND medid = " & objDR2.Item("medid")
                                    objCmd = New OdbcCommand(strSQLextJobMedarbTimer, objConn)
                                    objDR3 = objCmd.ExecuteReader
                                    Dim m As Integer = 0
                                    If objDR3.Read() Then
                                        m = 1
                                        expTxt = expTxt & objDR3.Item("timer") & ";"
                                    End If
                                ...
                                Dim strSQLextakt As String = "SELECT aktid, a.navn as aktnavn FROM ressourcer_md as r LEFT JOIN aktiviteter as a ON (a.id = r.aktid) WHERE jobid =" & objDR.Item("jobid") & " AND r.aktid <> 0 GROUP BY aktid"
                                ...
                                While objDR2.Read()
                                    ...
                                    'Dim strSQLexTtimer As String = "SELECT sum(timer) as timer, medid FROM ressourcer_md WHERE aktid = " & objDR2.Item("aktid") & " GROUP BY medid"
                                    'objCmd = New OdbcCommand(strSQLexTtimer, objConn)
                                    'objDR3 = objCmd.ExecuteReader
                                    'expTxt = ""
                                    'While objDR3.Read()

                                    'expTxt = expTxt & objDR3.Item("timer") & " " & objDR3.Item("medid") & ";"

                                    'End While

                                    Dim strSQLextTimer As String = "SELECT medid FROM ressourcer_md as r LEFT JOIN job as j ON (j.id = r.jobid) WHERE j.jobstatus = 1 GROUP BY medid"
                                    objCmd = New OdbcCommand(strSQLextTimer, objConn)
                                    objDR3 = objCmd.ExecuteReader
                                    expTxt = ""
                                    While objDR3.Read()
                                        'expTxt = expTxt & objDR3.Item("medid") & " " & objDR2.Item("aktid") & ";"

                                        Dim strSQLextTimertot As String = "SELECT sum(timer) as timer FROM ressourcer_md WHERE aktid = " & objDR2.Item("aktid") & " AND medid = " & objDR3.Item("medid")
                                        objCmd = New OdbcCommand(strSQLextTimertot, objConn)
                                        objDR4 = objCmd.ExecuteReader
                                        Dim x As Integer = 0
                                        If objDR4.Read() = True Then
                                            x = 1
                                            expTxt = expTxt & objDR4.Item("timer") & ";"
                                        End If
                                        If x = 0 Then
                                            expTxt = expTxt & ";"
                                        End If
                ...
                Case 6 ' Rapporttype 0 og 1 'skal være else
                    ...
                    'lto = "outz"
                    Using writer As StreamWriter = New StreamWriter("D:\webserver\wwwroot\timeout_xp\wwwroot\ver4_22\inc\upload\" & lto & "\" & flname, False, Encoding.GetEncoding("iso-8859-1"))
