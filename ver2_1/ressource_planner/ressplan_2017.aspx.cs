...
public partial class ressplan_2017 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ////sqlquery = query;
        //connString = "Data Source=127.0.0.1;Database=timeout_intranet;User ID = root; Password=";
        ...
        string sortbyPresel = Request["sortbypresel"];
        if (sortbyPresel == "2")
        {
        ...
        string jobidPresel = Request["jobidpresel"];
        if (jobidPresel != "")
        {
            ddjobidPresel.Value = Request["jobidpresel"];
        }
        else
        {

            ddjobidPresel.Value = "0";
        }
    ...
    private static string GetSortByTypeQuery(int sortby)
    {
        ...
        if (sortby > 0)
        {
            ...
            {
                query = "SELECT DISTINCT(job.id), job.jobnavn, job.jobnr, kunder.Kkundenr, kunder.kkundenavn FROM job LEFT JOIN kunder ON kunder.`Kid` = job.`jobknr` WHERE job.id IS NOT NULL AND jobstatus = 1 ORDER BY kunder.kkundenavn";
            }
        ...
        if (dsData != null && dsData.Tables.Count > 0 && dsData.Tables[0].Rows.Count > 0)
        {
            ...
            for (int i = 0; i < dsData.Tables[0].Rows.Count; i++)
            {
                ...
                if (existingJob == null)
                {
                    ...
                    job.JobNo = Convert.ToInt32(drUsers["JobNo"]);
                    job.CustomerNo = Convert.ToInt32(drUsers["CustomerNo"]);
        ...
        try
        {
            ...
            if (dsData != null && dsData.Tables.Count > 0 && dsData.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dsData.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (sortby == 2)
                    {
                        ddText += " (" + Convert.ToString(drUsers["jobnr"]) + ")";

                        string compare = "'" + Convert.ToString(drUsers["kkundenavn"]) + " " + Convert.ToString(drUsers["Kkundenr"]) + "'";
                        if (oliverDD.IndexOf(compare) > -1)
                        {
                            int lastIndex = oliverDD.LastIndexOf("</optgroup>");
                            oliverDD = oliverDD.Insert(lastIndex, "<option value='" + value + "'>" + ddText + "</option>");
                        }
                        else
                        {
                            string optGroupLabel = Convert.ToString(drUsers["kkundenavn"]) + " " + Convert.ToString(drUsers["Kkundenr"]);
