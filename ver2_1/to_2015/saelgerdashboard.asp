...
<% if len(trim(request("FM_periode"))) <> 0 then
   ...
   usePeriode = 1 'måned
    ...
    if kundeid <> 0 then
    kundeSQLkri = " AND kid = " & kundeid
    else
    kundeSQLkri = " AND kid <> 0 " 
    end if
    ...
    <div class="container">
        ...
        <div class="well">
              <form action="saelgerdashboard.asp?sogsubmitted=1" method="POST">
                ...
                <div class="col-lg-2">Kunder:</div>
                <div class="col-lg-3">Projekter:</div>
                  ...
                  <div class="row">
                      ...
                      <div class="col-lg-2">
                       ...
                       <%           
                           ...
                           &" GROUP BY kkundenr ORDER BY kkundenavn, jobnavn" 
                           ...
                           %>
                            ...
                            <option value="0" <%=kSel%>>Alle</option>
                       <%
                                    ...
                                    if cint(kundeid) = cint(oRec3("kid")) then
                                    kSel = "SELECTED"
                                    else
                                    kSel = ""
                                    end if
                                    %>
                                    <option value="<%=oRec3("kid") %>" <%=kSel %>><%=oRec3("kkundenavn")%></option>
                                    <%
                   ...
                   <div class="col-lg-3">
                       ...
                       <%           
                           ...
                           strSQLKproj = "SELECT kkundenavn, kkundenr, jobnavn, jobnr, j.id AS jid, kid FROM job AS j LEFT JOIN kunder AS k ON (k.kid = j.jobknr) "_
                           &" WHERE risiko > -1 AND (j.salgsans1 = "& usemrn &" OR j.salgsans2 = "& usemrn &" OR j.salgsans3 = "& usemrn &" OR j.salgsans4 = "& usemrn &" OR j.salgsans5 = "& usemrn &") "& kundeSQLkri &" "_
                           ...
                           %>
                              <select name="FM_job" class="form-control input-small" multiple size="5">
        ...
        jobKnrSQLkri = replace(kundeSQLkri, "kid", "jobknr")


            '*** DB1 + DB2 - Valgte Medarb
            strSQLjobDB = "SELECT jobnavn, jobnr, SUM(budgettimer) AS budgettimer, SUM(jo_bruttofortj) AS jo_bruttofortj, SUM(jo_dbproc) AS jo_dbproc, SUM(jo_udgifter_ulev) AS jo_udgifter_ulev, SUM(jo_udgifter_intern) AS jo_udgifter_intern FROM job AS j WHERE risiko > -1 "& JobSQLkri & "" & jobKnrSQLkri &""
            ...
            if len(trim(grp_jo_bruttofortj)) <> 0 then
            grp_jo_bruttofortj = grp_jo_bruttofortj
            else
            grp_jo_bruttofortj = 0
            end if

            if len(trim(grp_jo_dbproc)) <> 0 then
            grp_jo_dbproc = grp_jo_dbproc
            else
            grp_jo_dbproc = 0
            end if

            if len(trim(grp_jo_db2)) <> 0 then
            grp_jo_db2 = grp_jo_db2
            else
            grp_jo_db2 = 0
            end if

            if len(trim(grp_jo_db2_proc)) <> 0 then
            grp_jo_db2_proc = grp_jo_db2_proc
            else
            grp_jo_db2_proc = 0
            end if
        ...
        '** Real. timer ALLE MEDARB - Kun fakturerbare - Valgte job for slagsansvarlige --> Kun Lukkede job??
         strSQlmtimer = "SELECT SUM(timer) AS sumtimer, SUM(timer*timepris) AS sumtimeOms, SUM(timer*kostpris) AS sumtimeKost FROM timer AS t WHERE tmnr <> 0 "& strSQLjobnrTimer &" AND ("& aty_sql_realHoursFakbar &") AND tdato BETWEEN '"& strDatoStartKri &"' AND '"& strDatoEndKri &"'"
        ...
        medarbAllSumKost = 0 
         ...
         if not oRec.EOF then
            ...
            medarbAllSumtimer = oRec("sumtimer")
            medarbAllSumOms = oRec("sumtimeOms")
            medarbAllSumKost = oRec("sumtimeKost")
           
           
         end if
         oRec.close 

        medarbAllSumDB1 = formatnumber(medarbAllSumOms - medarbAllSumKost, 0)

         '** Matforbrug ALLE MEDARB ***'
         strSQlmatforbrug = "SELECT SUM(matkobspris) AS summatkobspris FROM materiale_forbrug AS mf WHERE usrid <> 0 "& replace(JobSQLkri, "j.id", "jobid") &" AND forbrugsdato BETWEEN '"& strDatoStartKri &"' AND '"& strDatoEndKri &"'"
        
        'response.write strSQlmatforbrug
        'response.flush

        summatkobspris = 0
         oRec.open strSQlmatforbrug, oConn, 3
         if not oRec.EOF then

            summatkobspris = oRec("summatkobspris")
        ...
        medarbAllSumDB2 = (medarbAllSumDB1 - summatkobspris)
        if medarbAllSumDB2 <> 0 then
        medarbAllSumDB2 = formatnumber(medarbAllSumDB2, 0)
        else
        medarbAllSumDB2 = 0
        end if 
        ...
        '**********************************************************************************
            ...
            if medarbAllSumtimer <> 0 then
            
                if faktureringsgrad > 0 then
                faktureringsgrad_proc = (budgettimerProc / medarbAllSumtimer) * 100
                faktureringsgradCol = "success"
                faktureringsgradFortegn = "+"
                else
                faktureringsgrad_proc = (medarbAllSumtimer / budgettimerProc) * 100
       ...
       strSQlmtimerUtil = "SELECT SUM(timer) AS sumtimer FROM timer AS t WHERE tmnr = "& usemrn &" "& strSQLjobnrTimer &" AND ("& aty_sql_realHoursFakbar &") "& strSQLaktidsfomr &" AND tdato BETWEEN '"& strDatoStartKri &"' AND '"& strDatoEndKri &"' GROUP BY tmnr"
        ...
        if (ntimPer) <> 0 then
            ...
            medarbSumtimerUtilpostProc = formatnumber((medarbSumtimerUtilpost/donut100) * 100, 0)
             ...
             medarbSumtimerUtilpreProc = 0
            medarbSumtimerUtilprodProc = 0
            medarbSumtimerUtilpostProc = 0
            end if
        %>
        ...
        <div class="portlet-body">
            ...
            <div class="col-sm-6 col-md-3">
              <div class="row-stat">
                ...
                <h3 class="row-stat-value"><%=formatnumber(medarbAllSumDB1, 2)%></h3>
                <span class="label label-danger row-stat-badge"><%=formatnumber(jo_dbproc, 0)%> %</span> 
                  <br /> <span style="font-size:11px; color:#999999;">[ Oms. - Salgsomkostninger ]</span>
                    <!-- <br /> <span style="font-size:9px; color:#999999;">Budget: <%=formatnumber(jo_bruttofortj,2) %></span> -->
                  <br /><br />
                <p class="row-stat-label">DB2</p>
                <h3 class="row-stat-value"><%=formatnumber(medarbAllSumDB2, 2)%></h3>
                <span class="label label-succes row-stat-badge"><%=formatnumber(jo_db2_proc, 0)%>%</span>
                  <br /> <span style="font-size:11px; color:#999999;">[ Oms. - (Salgsomkostninger + Løn) ]</span>
            ...
            <div class="col-sm-6 col-md-3">
              <div class="row-stat">
                ...
                <span class="label label-<%=faktureringsgradCol %> row-stat-badge"><%=faktureringsgradFortegn &" "& formatnumber(faktureringsgrad, 0) %> t.</span>
                 <br /> <span style="font-size:9px; color:#999999;">Budget: <%=formatnumber(budgettimer, 2) %> Real.: <%=formatnumber(medarbAllSumtimer, 2) %> t.</span>
              </div> <!-- /.row-stat -->
            </div> <!-- /.col -->
            ...
            <div class="col-sm-6 col-md-3">
              <div class="row-stat">
                ...
                <h3 class="row-stat-value"><%=formatnumber(medarbSumtimerUtilprod, 0) %> t.</h3>
                <span class="label label-<%=utilGradCol %> row-stat-badge"><%=utilGradFortegn &" "& formatnumber(utilLevel, 0) %> %</span>
                  <br /> <span style="font-size:9px; color:#999999;">Norm.: <%=formatnumber(ntimPer,0) %> t.</span>
