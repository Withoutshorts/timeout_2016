...
if cint(useDatofilter) = 1 then
	...
	'if Request.Cookies("st_dag").haskeys then	
        ...
        'if session("mid") = 1 then 
        'Response.write thisfile & " HER<br>"
        'Response.write "useDatofilter" & useDatofilter
        'Response.end
        'end if

                '** sikrer interval ikke er større end 4 år til at starte med **'
                if dateDiff("d", strDag&"/"&strMrd&"/"&strAar, strDag_slut&"/"&strMrd_slut&"/"&strAar_slut, 2,2) > 1460 then

                nyStDato = dateAdd("d", -1460, strDag_slut&"/"&strMrd_slut&"/"&strAar_slut)
