...
if func = "red" then
		        ...
		        '*******************************************************************************'
		        '******************* Henter oprettede aktiviteter / JOB ******************************'
		        '*******************************************************************************'
                ...
                'if session("mid") = 1 then
                'Response.Write "<br>a: "&a& " sta_tomme " & sta_tomme & "showOld" & showOld & " tdato:  "&cDate(strTdato) & " dato:  "&cDate(strDato) & "<br>"
                'Response.flush
                'end if
                ...
                strSQLaftalejob = "SELECT jobnavn, jobnr, id AS jid, valuta FROM job j WHERE serviceaft = "& aftid & " AND jobstatus <> 0 AND serviceaft <> 0 "& ermedpaaFakSQL
                        ...
                        'if session("mid") = 1 then
                        'Response.write "strSQLaftalejob " & strSQLaftalejob & "<br>"
                        'end if
                        'Response.end
...
<input id="antal_x" name="antal_x" value="100" type="hidden" />
<%
...
for x = 0 to 100 '50
            ...
            '*** jobid må kun være med 1 gang SIKKERHED

          
            if instr(lastjobids, "#"& fl_jobaktid(x) &"#") <> 0 AND len(trim(fl_jobaktid(x))) <> 0 AND fl_jobaktid(x) <> 0 then  
            fl_jobaktid(x) = 0
            fl_antal(x) = 0
            fl_besk(x) = ""
            fl_vis(x) = ""
            ...
            '**** Start altid med 0 i antal
            if thisaktfunc(a) <> "red" AND lto = "epi2017" then
            fl_antal(x) = 0
            fl_vis(x) = ""
            end if
            ...
            'if session("mid") = 1 then
            'Response.write "jobid: "& fl_jobaktid(x) & "x: " & x & " fl_vis(x): "& fl_vis(x) &"<br>"
            'end if

           

            '*** Faste generelle ***'
            if len(trim(fl_belob(x))) = 0 then
            fl_belob(x) = 0
            else
            fl_belob(x) = fl_belob(x)
            end if

            if len(trim(fl_antal(x))) = 0 then
            fl_antal(x) = 0
            else
            fl_antal(x) = fl_antal(x)
            end if
			...
			& "&nbsp;<input type='button' name='beregn2_"&x&"_"&a&"_a' id='beregn2_"&x&"_"&a&"_a' value='Calc' style='font-size:8px' onClick='tjektimer("&x&","&a&"), setBeloebThis2("&x&","&a&"), hideborder2("&x&","&a&")';>"
		    'tjektimer("&x&","&a&"), andreEnhprs("&x&","&a&"), setBeloebThis2("&x&","&a&"), hideborder2("&x&","&a&")

            jobnrTjk = 0
            if fl_jobaktid(x) <> 0 AND len(trim(fl_jobaktid(x))) <> 0 then
            fl_jobaktid(x) = fl_jobaktid(x)
            else
            fl_jobaktid(x) = 0
            end if

            strSQLhtisjobdoubletjk = "SELECT jobnr FROM job WHERE id = " & fl_jobaktid(x)
            oRec8.open strSQLhtisjobdoubletjk, oConn, 3
            if not oRec8.EOF then
            
            jobnrTjk = oRec8("jobnr")

            end if
            oRec8.close

            if jobnrTjk <> 0 then
            strAktsubtotal = strAktsubtotal &"<span style=""color:#999999; font-size:9px;"">Job: "& jobnrTjk &"</span></td>"
            else
            strAktsubtotal = strAktsubtotal &"</td>"
            end if
        ...
        lastjobids = lastjobids & ",#"& fl_jobaktid(x) &"#"

        'end if 'if instr(lastjobids, "#"& fl_jobaktid(x) &"#")
...
<div id="aktdiv_2" style="position:absolute; visibility:hidden; display:none; top:5672px; width:720px; left:5px; border:0px #8cAAe6 solid;">
