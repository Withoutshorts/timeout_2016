        ...
        for p = 0 to phigh - 1
                      ...
                      if p = 0 then 'Total på job / akt
                        ...
                        grpBy = "jobid"
                         ...
                         strSQLjobFcGt = "SELECT SUM(timer) AS sumtimer FROM ressourcer_md WHERE jobid = " & jobid &" "& aktKri &" AND (medid <> 0) AND (aar = "& h1aar &" AND md = "& h1md &") GROUP BY "& grpBy 
                       ...
                       %>
                        ...
                        <td style="text-align:left;"><%=thisJobAktSaldoTxt %></td>
            ...
            <%if cint(visKunFCFelter) = 0 OR cint(visKunFCFelter) = 1 then %>
             <td style="text-align:right;">
            <input type="text" id="afd_jobakt_<%=jobid%>_<%=aktid %>_<%=p %>" class="form-control input-small afd_jobakt afd_jobakt_<%=p %> afd_jobakt_<%=jobid%>_<%=aktid %>" style="width:60px; text-align:right; background-color:<%=fcBgCol%>;" value="<%=progrpTotTxt %>" DISABLED />
            </td>
            <%end if %>
           ...
           <%end if %>
            ...
            <%end if %>
             ...
             <%if (cint(visKunFCFelter) = 0 OR cint(visKunFCFelter) = 3) then %>
            <td class="afd_p_<%=p%> afd_p" style="white-space:nowrap; visibility:<%=pvzb%>; display:<%=pdsp%>;"><input type="<%=h1h2type %>" name="FM_H1H2" id="mh12h_jobaktmid_<%=jobid%>_<%=aktid %>_<%=antalm(m,1) %>" class="form-control input-small fs_<%=jobid%>_<%=aktid %>_<%=p %>" style="width:45px; background-color:<%=bgTotfc%>;" value="<%=thisMedarbSaldoTxt%>" disabled /></td>
            <%end if %>
