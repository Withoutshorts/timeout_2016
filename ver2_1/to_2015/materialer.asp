...
<%
    ...
    <div class="container">
        <div class="portlet">
            <h3 class="portlet-title"><u>Materialer - <%=varbroedkrumme%></u></h3>
    ...
    <div class="container">
        <div class="portlet">
            ...
            <div class="portlet-body">
                ...
                <form action="materialer.asp?menu=mat" method="post">
                    <div class="well">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3"><input type="text" name="FM_sog" id="FM_sog" value="<%=sogVal%>" class="form-control <%=sogCls %>" placeholder="% Wildcard"></div>
                        </div>
                        <br />
                        ...
                        <div class="row">
                            ...
                            <%else %>
                            <input type="hidden" name="FM_mgrp" id="FM_mgrp" value="0" />
                            <%end if %>
                            ...
                            <%else %>
                            <input type="hidden" id="soglev" name="soglev"  value="0" />
                            <%end if %>
                            ...
                            <%else %>
                            <input type="hidden" name="FM_limitvarenr" id="FM_limitvarenr"  value="" />
                            <input type="hidden" id="FM_limit" name="FM_limit"  value="100" />
                            <%end if %>
                        </div>
                ...
                if browstype_client <> "ip" then
                    lager_list_visgruppe = 1
                    lager_list_vislokation = 1
                    lager_list_visbillede = 1
                    lager_list_visnavn = 1
                    lager_list_visvarenr = 1
                    lager_list_visoddato = 1
                    lager_list_vislagerantal = 1
                    lager_list_visminlager = 1
                    lager_list_visindkobspris = 1
                    lager_list_vissalgspris = 1
                    lager_list_vislev = 1
                    tablezoom = "100%"
                else
                    lager_list_visgruppe = 0
                    lager_list_vislokation = 0
                    lager_list_visbillede = 0
                    lager_list_visnavn = 1
                    lager_list_visvarenr = 1
                    lager_list_visoddato = 0
                    lager_list_vislagerantal = 1
                    lager_list_visminlager = 0
                    lager_list_visindkobspris = 1
                    lager_list_vissalgspris = 1
                    lager_list_vislev = 0
                ...
                <table class="table dataTable table-striped table-bordered table-hover ui-datatable" style="zoom:<%=tablezoom %>;">

                    <thead>
                        <tr>
                            <%if cint(lager_list_visgruppe) = 1 then %>
                            <th>Gruppe
                                <%if level <= 2 OR level = 6 then %>
		                        <br />(Ava. %)
		                        <%end if %>
                            ...
                            <%if cint(lager_list_vislokation) = 1 then %>
                            <th>Lokation</th>
                            <%end if %>

                            <%if cint(lager_list_visbillede) = 1 then %>
                            <th>Billede</th>
                            <%end if %>

                            <%if cint(lager_list_visnavn) = 1 then %>
                            <th>Navn <br /> Note</th>
                            <%end if %>

                            <%if cint(lager_list_visvarenr) = 1 then %>
                            <th>Varenr.</th>
                            <%end if %>

                            <%if cint(lager_list_visoddato) = 1 then %>
                            <th>Opd. dato</th>
                            <%end if %>

                            <%if cint(lager_list_vislagerantal) = 1 then %>
                                <%if browstype_client <> "ip" then %>
		                        <th>På lager (ordre)</th>
                                <%else %>
                                <th>Lager</th>
                                <%end if %>
                            <%end if %>

                            <%if cint(lager_list_visminlager) = 1 then %>
		                    <th>Min. lager</th>
                            <%end if %>


		                    <%if level <= 2 OR level = 6 then %>
                                <%if cint(lager_list_visindkobspris) = 1 then %>
                                    <%if browstype_client <> "ip" then %>
		                                <th>Indkøbs-pris</th>
                                    <%else %>
                                        <th>I-pris</th>
                                ...
                                <%if cint(lager_list_vissalgspris) = 1 then %>
                                    <%if browstype_client <> "ip" then %>
		                                <th>Salgs-pris</th>
                                    <%else %>
                                        <th>S-pris</th>
                            ...
                            <%if cint(lager_list_vislev) = 1 then %>
		                    <th>Pri. lev.</th>
		                    <%end if %>
                    ...
                    <tbody>
                        ...
                        <tr bgcolor="<%=bgthis%>">
                            ...
                            <%if cint(lager_list_visnavn ) = 1 then %>
                            <td>
		                        <%if (level <= 2 OR level = 6) AND browstype_client <> "ip" then %>
                            ...
                            dicim = 2
                                ...
                                <%if cint(lager_list_visindkobspris ) = 1 then %>
		                        <td style="text-align:right;"><%=formatnumber(oRec("indkobspris"),dicim) %></td>
                                <%end if %>

                                <%if cint(lager_list_vissalgspris  ) = 1 then %>
		                        <td style="text-align:right;"><%=formatnumber(oRec("salgspris"),dicim) %></td>
                                <%end if %>
		                    <%end if%>
