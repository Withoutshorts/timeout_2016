...
$(document).ready(function () {
    ...
    if ($(document).width() < 1600) {
        $("#favoritcontent").addClass("smallscreen")
    }

    if ($(document).width() < 1570) {
        $("#favoritcontent").removeClass("smallscreen")
        $("#favoritcontent").addClass("tinyscreen")
    }

    $("#showcaldenderdropdown").click(function () {
        document.cookie = "calendar2020=0";
        location.reload();
    });

    $("#showcaldenderfixed").click(function () {
        document.cookie = "calendar2020=1";
        location.reload();
    });


    $(".timer_flt").keyup(function () {

        var modalid = this.id
        var idlngt = modalid.length
        var idtrim = modalid.slice(9, idlngt)


        var str = $("#FM_timer_" + idtrim).val();
        ...
        var norm = $("#FM_norm_" + idtrim).val();

        //alert($.isNumeric(str))


        if (str == "1d") {
            $("#FM_timer_" + idtrim).val(norm);
        } else {
            ...
            if (n == -1) {
                ...
                if ($.isNumeric(str) != true) {
                    //$("#FM_timer_" + idtrim).val('####');
                    var sstr = $("#FM_timer_" + idtrim).val();
                    sstr = sstr.replace(",", ".")
                    var idlngt = sstr.length
                    //alert(idlngt)
                    var sstr = sstr.slice(0, idlngt - 1)

                    if ($.isNumeric(sstr) != true) {
                        $("#FM_timer_" + idtrim).val('');
                    } else {
                        sstr = sstr.replace(".", ",")
                        $("#FM_timer_" + idtrim).val(sstr);
                    }
    ...
    $('.sp_tr_jobid').click(function () {
        ...
        if ($('.tr_jobid_' + thisval).css("visibility") == "hidden") {
            $('.tr_jobid_' + thisval).css('visibility', 'visible')
            $('.tr_jobid_' + thisval).css('display', '')
            $('#sp_tr_jobid_' + thisval).removeClass("fa-plus");
            $('#sp_tr_jobid_' + thisval).addClass("fa-minus");
            $('.tr_jobid_' + thisval).show(200)

            $.cookie('sp_tr_jobid_' + thisval, '1');

        } else {

            $('.tr_jobid_' + thisval).hide(200)
            $('.tr_jobid_' + thisval).css('display', 'none')
            $('.tr_jobid_' + thisval).css('visibility', 'hidden')
            $('#sp_tr_jobid_' + thisval).removeClass("fa-minus");
            $('#sp_tr_jobid_' + thisval).addClass("fa-plus");

            $.cookie('sp_tr_jobid_' + thisval, '0');
    ...
    //alert($('#alljobids').val())
    thisAllJobidsArr = $('#alljobids').val().split(',');

    jQuery.each(thisAllJobidsArr, function () {
        //$('#' + this).removeClass('collapsed');

        //alert(this)
        if ($.cookie('sp_tr_jobid_' + this) == '1') {
            $('.tr_jobid_' + this).css('visibility', 'visible')
            $('.tr_jobid_' + this).css('display', '')

            $('#sp_tr_jobid_' + this).removeClass("fa-plus");
            $('#sp_tr_jobid_' + this).addClass("fa-minus");
    ...
    for (i = 0; i < 10000; ++i) {
        ...
        if ($.cookie('sp_tr_jobid_' + i) == '1') {
            $('.tr_jobid_' + i).css('visibility', 'visible')
            $('.tr_jobid_' + i).css('display', '')
    ...
    */
    ...
    function sogjobogkunde(thisid) {
        ...
        varTjDatoUS_man = $("#jq_varTjDatoUS_man").val()
        ...
        //alert("medid" + jq_newfilterval + " jq_ign_projgrp" + jq_ign_projgrp)

        $.post("?jq_newfilterval=" + jq_newfilterval + "&jq_ign_projgrp=" + jq_ign_projgrp + "&varTjDatoUS_man=" + varTjDatoUS_man + "&jq_medid=" + jq_medid, { control: "FN_sogjobogkunde", AjaxUpdateField: "true" }, function (data) {
            //alert("cc")
            $("#dv_job").html(data);
