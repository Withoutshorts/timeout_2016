...
$(document).ready(function () {
    ...
    $("#FM_sogkunde").keyup(function () {


        
        thisSelVal = $("#FM_sogkunde").val()

        $("#FM_kunde").css("visibility", "visible");
        $("#FM_kunde").css("display", "");

        //alert("HER 2: " + thisSelVal)
        $.post("?sogkunde=" + thisSelVal, { control: "FN_getKunder", AjaxUpdateField: "true" }, function (data) {
            //alert(thisSelVal)
            //$("#help").val(data);
            $("#FM_kunde").html(data);
    ...
    $("#FM_jobonoff").click(function () {
        //alert("HER")
        $("#form_kunde").submit();
    });


    $("#kFM_job, #kFM_aftale").change(function () {
        //alert("HER")
        $("#form_jobaft").submit();
    });
    ...
    function GetMatTilFak() {
        ...
        $.post("?jq_lto=" + jq_lto.val() + "&jq_inttype=" + jq_intType.val() + "&jq_id=" + jq_id.val() + "&jq_stdato=" + jq_stdato.val() + "&jq_sldato=" + jq_sldato.val() + "&jq_func=" + jq_func.val() + "&jq_valutaValkurs=" + valutaValkurs.val() + "&jq_valutafaklabel=" + valutaValLabel + "&jq_kuneks=" + viskuneks + "&jq_ignper=" + ignper + "&jq_jobid=" + jobid_val.val() + "&jq_aftid=" + aftid_val.val() + "&jq_fastpris=" + jq_fastpris, { control: "FN_getMatTilFak", AjaxUpdateField: "true", cust: kid_val.val() }, function (data) {
            ...
            $("#divbelobtot_umoms").html("(" + totbel_umoms + " " + valutaValLabel + ") ");
    ...
    $("#FM_usealtadr_all").click(function () {
        GetNyeKpers();
    });
    ...
    function GetNyeKpers() {
        ...
        var usealtadr_all = "" //$("#FM_usealtadr_all").val()

        $.post("?att_val=" + att_val.val() + "&FM_usealtadr_all=" + usealtadr_all , { control: "FN_getKpers", AjaxUpdateField: "true", cust: kid_val.val() }, function (data) {
            //$("#fajl").val(data);
            $("#FM_altadr").html(data);
            $("#FM_att").html(data);
        });
...
function setmTxt(aktid, n, x) {
    ...
    if (kCode == 37){ 
		...
		if (kCode == 39){
            ...
            //alert("KURTA: " + belobakttot)
            //alert("KURTB: "  + belobmattot)
                    
                    belobtotal = belobakttot + belobmattot
            //alert("KURTC: " + belobtotal)
				    belBobtotal = Math.round(belobtotal * 100) / 100
                    ...
                    document.getElementById("divbelobtot_umoms").innerHTML = "(" + document.getElementById("FM_beloeb_umoms").value + " " + valutaKodeFak + ") "
...
function hideborder2(x, a) {
	...
	var valutaKodeFak = document.getElementById("FM_valuta_all_1").options[document.getElementById("FM_valuta_all_1").selectedIndex].text
	           ...
	           // alert(matBelobialt) // + " " + matBelobialt_umoms)
	            ...
	            document.getElementById("divbelobtot").innerHTML = "<b>"+ valutaKodeFak + "</b>"
	            document.getElementById("divbelobtot_umoms").innerHTML = "(" + beloeb_umoms + " " + valutaKodeFak + ") "
