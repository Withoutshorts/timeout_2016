...
$(document).ready(function () {
    ...
    $("#FM_product_group").change(function () {

        //alert("OK2")

        beregnFreightPrice_pc();

        //alert("OK")
        tjekkerKost_ogSalgspris_pc();

    });


    function beregnFreightPrice_pc() {

        matgrp = $("#FM_product_group").val();
        valuta = $("#FM_valuta_freight_price_pc_valuta").val();

        levbetint = $("#FM_levbetint").val();
        levlevbetint = $("#FM_levlevbetint").val();
        transport = $("#FM_transport").val()

        //alert("matgrp: " + matgrp + " valuta: " + valuta)

        $.post("?matgrp=" + matgrp + "&valuta=" + valuta + "&levbetint=" + levbetint + "&levlevbetint=" + levlevbetint + "&transport=" + transport, { control: "FN_hentfragpris", AjaxUpdateField: "true", cust: 0 }, function (data) {

            if (data != "DONOTUPDATE") {
                $("#freight_pc").val(data);
                $("#freight_pc").css("background-color", "")
            } else {
                $("#freight_pc").css("background-color", "lightpink")
            }
    ...
    $("#FM_levbetint, #FM_origin").change(function () {

        if ($("#FM_levbetint").val() == "22") {
            $("#tax_pc").val(0)
            $("#tax_pc").css("background-color", "lightpink")
        } else {
            $("#tax_pc").css("background-color", "")
        }

        //alert($("#FM_levbetint").val())

        if ($("#FM_levbetint").val() == "2") {

            if ($("#FM_origin").val() == "Turkey" || $("#FM_origin").val() == "Bangladesh" || $("#FM_origin").val() == "Denmark") {
                $("#tax_pc").val(0)
                $("#tax_pc").css("background-color", "lightpink")
            }

            if ($("#FM_origin").val() == "China" || $("#FM_origin").val() == "Vietnam") {
                $("#tax_pc").val(12)
                $("#tax_pc").css("background-color", "lightpink")
            }

            if ($("#FM_origin").val() == "India") {
               
                $("#tax_pc").val('9,6')
                $("#tax_pc").css("background-color", "pink")
            }
    ...
    $("#FM_levlevbetint").change(function () {
        beregnFreightPrice_pc();
        tjekkerKost_ogSalgspris_pc();
    });

    $("#FM_transport").change(function () {
        //alert("HER")
        beregnFreightPrice_pc();
        tjekkerKost_ogSalgspris_pc();
    });
    ...
    function tjekkerKost_ogSalgspris_pc() {
    ...
    function beregn_order() {
        ...
        sales_price_pc = (sales_price_pc / 1 * valuta_kurs_s / 1) / 100
           ...
           // Old: (Order Qty. * Cost. PC * TAX PC) + (Freight PC * Order Qty.)
           // New 12.12.2019: ((cost. PC + Freight PC)*Order Qty.) * TAX
        ...
        if (usenew == 0) {
            ...
            totalcostprice = 0
            totalcostprice = ((cost_price_pc * 1 + freight * 1) * (tax / 1 + 100 / 1) / 100)
        }
        ...
        //alert("Beregner ordre")

        //$("#jo_dbproc_label").css("background-color", "lightgreen")
