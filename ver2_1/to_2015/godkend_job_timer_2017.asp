...
<%
 '**** Søgekriterier AJAX **'
    ...
    if multi = "1" then
    strSQLgodkend = strSQLgodkend &" WHERE "& tidsSQLKri
    else
    strSQLgodkend = strSQLgodkend &" WHERE tid = "& tid
    end if
    ...
    select case filterThis
        ...
        case "3"
        filterSQL = " AND godkendtstatus = 0"
        case else
        filterSQL = ""
    end select
                            ...
                            <div class="col-lg-6"><br /><%=godkend_txt_002 %>:
                                ...
                                <select name="FM_job" id="FM_job" <%=progrpmedarbDisabled  %> class="form-control input-small" onchange="submit();">
                                  ...
                                  <%end select
                                    ...
                                    strSQLneutraltimer = "SELECT sum(timer) as timer FROM timer WHERE tjobnr ="& oRec3("jobnr") & " AND godkendtstatus = 0"
                                    ...
                                    jobnavnid = oRec3("jobnr") &" - "& oRec3("jobnavn") &"&nbsp - &nbsp"& neutraltimer &" t."
                                    else
                                    jjobnavnid = oRec3("jobnr") &" - "& oRec3("jobnavn")
                                    end if
                                    case else 
				                    if neutraltimer <> 0 then
                                    jobnavnid = oRec3("jobnavn") & " ("& oRec3("jobnr") &")" &"&nbsp - &nbsp"& neutraltimer &" t."
                                    else
			...
			next %>
                      ...
                      <table style="background-color:white;" class="table dataTable table-striped table-bordered table-hover ui-datatable">
                          ...
                          <tbody>
                              ...
                              <%
                                  ...
                                  %>
                                        ...
                                        strSQLakt = "SELECT tid, taktivitetid, taktivitetnavn, timer, tdato, godkendtdato, godkendtstatus, godkendtstatusaf, timerkom FROM timer WHERE ("& aty_sql_realhours &") AND tjobnr = '"& Strjobid & "' AND tmnr ="& medarbid(m) & " AND tdato BETWEEN '"& startdato &"' AND '"& slutdato &"' "& filterSQL &" ORDER BY taktivitetnavn"
                                        'response.Write strSQLtimer
                                        'response.Flush
                                        ...
                                        <tr>                                      
                                           ...
                                           ' oRec2.open strSQLakttimer, oConn, 3
                                            ...
                                            <td style="text-align:center">                                               
                                                <input type="radio" id="godkendstatus_<%=medarbid(m) %>_<%=aktid %>_<%=tid %>" class="godkendbox godkendbox_<%=medarbid(m)%>" name="godkendbox_<%=tid %>" value="<%=medarbid(m) %>_<%=aktid %>_<%=tid %>" <%=aktgodkendtstatus1CHK %> /></td> 
                                            <td style="text-align:center">                                               
                                                <input type="radio" id="afvisstatus_<%=medarbid(m) %>_<%=aktid %>_<%=tid %>" class="afvisbox afvisbox_<%=medarbid(m)%>" name="godkendbox_<%=tid %>" value="<%=medarbid(m) %>_<%=aktid %>_<%=tid %>" <%=aktgodkendtstatus2CHK %> /></td>
                                  ...
                                  <td style="text-align:right" colspan="3">Comment for declined<br /><textarea id="decline_comment_<%=medarbid(m) %>" name="decline_comment" class="form-control input-small"></textarea>
                                      <br />
                                        <a id="sendemail_<%=medarbid(m) %>" class="sendemail">Send email with declined info >></a>
                         ...
                         <!-- <tfoot>
                          ...
                          <tbody>
                             <%
                            strSQLhentakt = "SELECT navn, id FROM aktiviteter AS a LEFT JOIN timer as t ON (a.id = t.TAktivitetId) WHERE job ="& jobid & " AND t.tdato BETWEEN '"& startdato &"' AND '"& slutdato &"' "&filterSQL
                           ...
                           <tr id="aktvisfont_<%=aktid %>">
                               ...
                               <td style="text-align:center">
                                   <%
                                       strSQLhenttimer = "SELECT sum(timer) as timer FROM timer WHERE tjobnr = "& Strjobid &" AND TAktivitetId ="& aktid & filterSQL &" AND tdato BETWEEN '"& startdato &"' AND '"& slutdato &"'"
                               ...
                               <td style="text-align:center">
                                   <%
                                        gktimerperiode = 0
                                        strSQLtimer = "SELECT sum(timer) as Timer FROM timer WHERE ("& aty_sql_realhours &") AND tjobnr = '"& Strjobid & "' AND TAktivitetId ="& aktid & " AND tdato BETWEEN '"& startdato &"' AND '"& slutdato &"' AND godkendtstatus = 1"
                                        'response.Write strSQLtimer
                                        'response.Flush
                           ...
                           <tbody style="display:none" id="tr_medarbliste_<%=aktid %>">
                                 <%
                                     m = 0
                                     strSQLMedarb = "SELECT Tmnavn, Tmnr, Tid, godkendtstatusaf, godkendtdato, Tdato, timer, godkendtstatus, timerkom FROM timer WHERE tjobnr = "& Strjobid &" AND TAktivitetId ="& aktid &" AND tdato BETWEEN '"& startdato &"' AND '"& slutdato &"' "&filterSQL&" ORDER BY Tmnavn"                                   
                                    ...
                                    <tr>
                                      ...
                                      <td style="text-align:right" colspan="3">Comment for declined<br /><textarea id="decline_comment_<%=lastTmnr %>" name="decline_comment" class="form-control input-small"></textarea>
                                      <br />
                                        <a id="sendemail_<%=lastTmnr %>" class="sendemail">Send email with declined info >></a>
                                        <!--<div id="sendemail_send_<%=lastTmnr %>">Mail send!</div>-->
                                        <input type="hidden" value="" id="decl_tids_<%=lastTmnr %>" />
                                        <input type="hidden" value="<%=lastTmnr %>" id="decl_tids_mid_<%=lastTmnr %>" class="decl_tids" />
                                      </td>
                                    </tr>
                                    <%end if %>
                                    ...
                                    <tr>                                      
                                        ...
                                        <%
                                            if cdbl(lastTmnr) <> cdbl(oRec2("Tmnr")) then
                                                    akTimerTot = 0
                                                    strSQLakttotal = "SELECT taktivitetnavn, sum(timer) as Timer FROM timer WHERE TAktivitetId ="& aktid &" AND ("& aty_sql_realhours &") AND tjobnr = '"& Strjobid & "' AND tmnr ="& oRec2("Tmnr") & filterSQL &" AND tdato BETWEEN '"& startdato &"' AND '"& slutdato &"'"
                                        ...
                                        <td style="text-align:center">                                             
                                                <input type="radio" id="godkendstatus_<%=oRec2("Tmnr") %>_<%=aktid %>_<%=oRec2("Tid") %>" class="godkendbox godkendbox_<%=oRec2("Tmnr")%>" name="godkendbox_<%=oRec2("Tid") %>" value="<%=oRec2("Tmnr") %>_<%=aktid %>_<%=oRec2("Tmnr") %>" <%=aktgodkendtstatus1CHK %> />
                                            <!--<input type="radio" class="godkendmedarb_akt" id="godkendmedarbakt_<%=oRec2("Tmnr") %>_<%=aktid %>_<%=oRec2("Tid") %>" />-->
                                        </td>
                                        <td style="text-align:center">
                                            <input type="radio" id="afvisstatus_<%=oRec2("Tmnr") %>_<%=aktid %>_<%=oRec2("Tid") %>" class="afvisbox afvisbox_<%=oRec2("Tmnr")%>" name="godkendbox_<%=oRec2("Tid") %>" value="<%=oRec2("Tmnr") %>_<%=aktid %>_<%=oRec2("Tid") %>" <%=aktgodkendtstatus2CHK %> />
                                        </td>
                                        <td style="text-align:center">
                                            <%
                          ...
                          <tfoot>
                              <tr>
                                  ...
                                  <th style="text-align:center">
                                      <%
                                          strTotalTimer = "SELECT sum(timer) as timer FROM timer WHERE ("& aty_sql_realhours &") AND tjobnr = '"& Strjobid & "'" & filterSQL &" AND tdato BETWEEN '"& startdato &"' AND '"& slutdato &"'"
